...
このPCでは **`Start-App.bat`** または共通ホーム `../AppHome` の「KnowledgeApp」から起動できます。隣接する `KnowledgeApp-builds/Start-App.ps1` が既存の検証済み0.7.3単一exeを照合して起動します。通常配布exeと同じ保存先・単一起動制御を使います。ホーム登録は1件にまとめ、旧インストールや旧Tauri版へ自動で切り替えません。

現行版は**0.7.3**です。FAQタブが少ないときの大きな間隔を解消し、左から詰めて並べます。0.7.2の起動改善とインストール不要の単一exe配布を維持し、既存のFAQ・認証・設定は保持します。0.7.1で行った白画面の修正は[修正・検証記録](起動時白画面修正_0.7.1.md)に残しています。
...
**日常から仕事まで、セキュアにFAQナレッジを活用できる。** KnowledgeAppは、Windows 11で利用する、一人用のローカルFAQ・ナレッジ管理アプリです。利用場所やFAQの題材を限定せず、必要な知識を蓄積・検索・更新できます。

通常のFAQ検索・閲覧・手動編集・保存はPC内で完結します。利用者別のローカル保存、認証・権限、確認を伴う外部URLの参照、対象を選ぶCodexへの明示委譲により、利用者が情報の扱いを管理できる構成です。利用環境の情報管理規定に従って運用してください。

**大前提：実際のFAQ・データベース・パスワードなどの利用者データや認証情報は、GitHubを含むGitリポジトリへ絶対にコミットしません。** 公開・非公開を問わず、DBのWAL／SHM、添付画像、履歴、バックアップ、CSV／JSONエクスポート、Codex提案・委譲、ログ、外部資料、APIキー・トークン・秘密鍵・緊急認証情報も対象です。文書・ソース・画像への転記も禁止します。Gitで管理するのはプログラム、設計文書、秘密を含まない設定例、実データを含まない開発用フィクスチャー、配布用静的素材だけです。

## 現在の開発状況

2026-09-06の利用者承認により、C#版を正式な利用・開発の主系とします。現行版は`0.7.3`です。WPF＋WebView2から現行React／Tiptap UIを使い、以後のバックエンド改修はC#で行います。旧Tauri／Rust版はバックアップ互換などの回帰確認用として保持します。

利用者から今回のPCでの復元・確認完了の報告を受領し、C#正式版への本番切替を完了しました。対象PCでの切替や実プラグイン往復など、未報告・未実施の確認を合格へ置き換えるものではありません。現在の確認範囲は[第17段階の切替記録](src-csharp/第17段階_CSharp正式切替記録.md)を参照してください。
...
文書の入口は[文書整理ガイド](文書整理ガイド.md)、未確認事項・改善候補の進行管理は[今後の課題](今後の課題.md)へ集約しました。旧版の試験書・計画書37件は原文を外部保管済みです。削除は利用者が行うため、削除前の現行ツリーに残っていても現在の起動手順や進捗判定には使いません。
...
- 管理者の英数字16文字の復旧キーによる新パスワード再設定（メール不要・固定緊急認証は廃止）
...
- インストール不要の単一exe配布、補助方式のNSISインストーラーと配布物混入検査
- Gitコミット前とGitHub Actionsでの利用者データ混入検査
- ソース・設定・設計文書のUTF-8（BOMなし）・LF統一と自動検査

専用HTML手順書管理は実装せず、外部管理のHTML、Excel、PDF、Wordなどをコピー用パスまたはHTTP/HTTPS参考URLで案内します。

アプリ外で管理する外部資料本体はKnowledgeAppへ取り込まず、検索、存在確認、リンク切れ検査、フルバックアップの対象に含めません。外部資料の配置、アクセス権、更新、安全性、バックアップは資料管理者側の運用に従ってください。FAQ本文に保存した参照先文字列と参考URLは、通常のFAQデータとしてバックアップ・復元されます。

## 利用を始める

**通常配布はインストール不要の単一exe版です。** `KnowledgeApp.CSharp.exe`だけをローカルフォルダーへコピーして起動します。.NET 10は内蔵し、WebView2 Runtimeは対象PCに必要です。FAQ・認証・設定はインストーラー版と同じC#専用LocalApplicationDataへ保存するため、exeの移動や差し替えで失われません。初回起動時は同梱ファイルを利用者の一時領域へ展開します。詳細は[単一exe版の案内](scripts/installer/StandaloneReadme.ja.txt)を参照してください。

1. 配布された`KnowledgeApp.CSharp.exe`をPCの専用ローカルフォルダーへコピーします。Windows 11 x64とWebView2 Runtimeが必要です。取得元とSHA-256を確認し、利用環境の実行・配布規定に従ってください。バックアップやCodex連携先とは別のフォルダーを使用します。
2. コピーしたexeをダブルクリックして起動します。既存のC#データがある場合は、現在のログインID・パスワードを使います。新規の専用データ領域だけログインID`0000`、パスワード空欄で開始します。旧版のFAQを引き継ぐ場合は、次の「旧Tauri版からの切り替え」を先に行ってください。
3. 「利用者の管理」で初期管理者の「パスワード再設定」を選び、利用環境の規定に沿うパスワードへ変更します。
...
6. 外部資料のパスは回答内のコピー用テキスト枠へ、Web資料はHTTP/HTTPS参考URLへ設定します。ローカル・UNCパスは自動で開かず、利用者がコピーして利用環境で許可された方法で参照します。
...
詳しい初回設定、定期バックアップ、更新前確認、外部資料の責任範囲は[KnowledgeApp運用手順](KnowledgeApp運用手順.md)を参照してください。旧版のリリースノート・試験記録は[文書整理ガイドの外部保管先](文書整理ガイド.md#2-履歴文書の保存先)で履歴として保持します。
...
旧版向けプラグインはC#の保存先へ自動で切り替わりません。C#対応版を使用し、保存先の不一致を旧版領域の直接読取やファイルコピーで回避しないでください。プラグインのソース試験と、対象PC／Codexへ導入された実プラグインの確認は別に扱います。
...
上記は主な試験です。CIは認証、復元、Codex、UI、安全境界、配布などの専用試験をすべて実行します。`ProductionCheck`を含む自動試験は新規の合成一時領域を使い、本番FAQを読みません。対象PC・実データ・実プラグインの未実施確認を自動試験の合格と混同しません。
...
### 補助方式：NSISインストーラーを生成する

通常配布は次節の単一exeを使います。利用環境でインストーラーが必要な場合の手順です。
...
生成先は各スクリプトの結果に返されます。成果物は`KnowledgeApp-CSharp-0.7.3-win-x64.zip`と`KnowledgeApp-CSharp-0.7.3-setup.exe`です。通常ZIPはフォルダー全体で使用します。下記の単一exe版は別の生成手順です。NSISコンパイラーはPATHまたは`KNOWLEDGEAPP_NSIS_COMPILER`で明示し、生成スクリプトは自動取得しません。CIではNSISを独立して準備します。

GitHub Actionsの通常CIは単一exe、C#パッケージ、NSISを保存します。通常配布では単一exeを選びます。旧Rustの検査・相互試験は手動起動の別ワークフローへ分離しています。パッケージのSHA-256は整合性確認用であり、コード署名や利用環境の配布承認の代替ではありません。

### 通常配布：単一exeを1コマンドでビルドする

```powershell
npm run build:exe
```

開発PCにはNode.js/npm、.NET 10 SDK、PowerShell 7が必要です。初回だけ`npm ci`を実行します。`build:exe`は現在のUIをビルドし、C#の依存復元・ビルド、単一exe生成、合成DBのログイン表示まで順に検証します。開発用の`npm run build`はUIビルド専用のままです。

省略時はWindowsのドキュメントフォルダーの`KnowledgeApp_Releases/0.7.3/build-日時-識別子/`へ出力し、最後に配布exeの絶対パスを表示します。出力先を指定する場合は`npm run build:exe -- -OutputDirectory C:/KnowledgeApp-builds/new-release`とします。リポジトリ内・既存出力先はビルド前に拒否します。

配布するのは出力先の`standalone/KnowledgeApp.CSharp.exe`だけです。.NETとUI、SQLite、WebView2ローダー、保存先確認用の補助exe、依存ライセンス案内を含みます。WebView2のブラウザー本体は含めません。作成時に既存パッケージの許可リストとUIハッシュを検査し、単一exeを合成一時領域へ展開して同梱依存、保存先境界、ログイン表示・終了を確認します。本番・固定検証DBは開きません。開発用`bin/Release`のexeを単体配布に使わないでください。別の端末で配布済みexeだけを再検査する場合は`./scripts/Test-CSharpStandaloneExe.ps1 -Executable <exeの絶対パス>`を使用します（検査には.NET SDKが必要です）。

`STANDALONE-MANIFEST.json`にSHA-256と検査結果、`SHA256SUMS.txt`に照合用ハッシュ、`STANDALONE-README.txt`に利用手順を出力します。これらはexeと別の配布案内で、実行時に隣接する必要はありません。起動速度を優先してexe圧縮・トリミングは使用しません。全内容展開は.NET 10の互換モードを使用するため、.NETのメジャー更新時には再確認します。インストーラー版と同じデータ・単一起動制御を使い、任意のデータ保存先や新しいCLI引数は追加しません。

0.4.3までのSurface全体試験結果を保持し、0.4.4以降はSurfaceで再試験しません。最初は実FAQへ接続しない検証用Windows利用者と合成FAQを使用します。2026-08-16付で、一人利用試行では対象コミット、起動、利用者データ分離、通常・緊急認証、ローカルバックアップ・復元、Git混入防止を最低確認とし、正式配布向けの追加確認は延期しました。0.3.3～0.4.3の実機・診断文書は過去候補の履歴として保持します。

旧版の対象環境検証手順・結果票は[外部保管した履歴](文書整理ガイド.md#2-履歴文書の保存先)です。C#版で未実施の対象PC起動や実データ復元は別に確認します。コード署名・正式配布経路・運用上の承認、広範な実機・表示・ネットワーク確認は、利用環境の規定が求める時点や複数人配布へ進む前に再開します。現在の対象と再開条件は[今後の課題](今後の課題.md)を参照してください。
...
FAQ、履歴、画像、バックアップ、エクスポート、ログ、アプリ外で管理する外部資料はGitへ登録しません。アプリは保存先の解決に失敗しても、作業フォルダの`./data`へ代替保存しません。旧版互換用の`manuals`領域は非破壊で保持しますが、現行画面から外部資料を取り込む用途には使用しません。
