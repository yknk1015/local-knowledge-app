...
Publicで公開するのは、プログラム、設計文書、設定例、開発用フィクスチャー、配布に必要な静的素材だけとする。利用者が作成したFAQ、DB、検索・閲覧履歴、添付画像、バックアップ、CSV・JSONエクスポート、診断ログ、Codex提案・委譲、会社管理の外部資料は公開しない。

この公開は、利用者本人による会社PCでの一人利用試行に向けたソース取得を目的とする。会社向け正式リリース、複数人配布、会社情報を含む運用の承認を意味しない。該当段階へ進む場合は会社規定に従い、公開範囲、配布経路、コード署名を再確認する。
