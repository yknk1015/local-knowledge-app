...
- 新規DBは第1版の初期構造を作成後、未適用マイグレーションを順番に実行して第6版にする。
- 第1～5版DBを開いた場合は、利用者データフォルダ内の同じDBへ各マイグレーションを順番に適用する。既存FAQの新着・更新は未設定、非表示はOFF、統合関係は未設定、作成者・更新者は初期管理者とする。
- 第1～5版DBを含む完全バックアップは復元対象として許可し、復元直後に同じ移行と初期管理者補完を実行する。現在のアプリより新しいDB版は復元しない。
- マイグレーションSQLはソース管理対象とするが、移行対象の実DBとバックアップは従来どおりリポジトリ外へ置く。
- 自動テストではOS一時フォルダに旧DBを作成し、第6版への移行、既存FAQの保持、初期管理者と監査列、分類説明、受付記録・提案履歴・統合関係を確認する。
