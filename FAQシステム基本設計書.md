...
|---|---|
| 文書名 | FAQシステム基本設計書 |
| 版 | 1.40（Surface実機リハーサル反映版） |
| 作成日 | 2026-08-08 |
| 上位文書 | `FAQシステム要件定義書.md` v1.30 |
| 対象リリース | MVP（初回リリース） |
...
複数値のFAQ検索情報、双方向の関連FAQ、未保存離脱警告、3種類の検索範囲、日本語分解・順位付け、同義語辞書、検索・閲覧履歴、JSON入出力、分類の同一親内での上下並べ替えは2026-08-16のリリース前開発単位で実装した。外部資料参照はコピー用テキスト枠とHTTP/HTTPS参考URLで実装済みとし、専用HTML手順書管理は2026-08-16の方針変更により対象外とする。本節は進捗を示すものであり、後続機能の要件を変更するものではない。
...
| Surface実機 | 出所を固定した旧版からの更新、合成データ、Excel、外部資料参照、表示倍率、単一起動、バックアップ・復元、アンインストール・再インストール |
...
- コミット`5f99114e`からクリーンビルドした0.2.0で合成FAQと画像を登録し、0.3.3へ上書き更新した際に、移行前安全バックアップ、DB第7版移行、初期管理者・監査情報・管理ID補完、既存データ保持をSurfaceで確認すること
...
6. FAQ実データへ接続しない専用Windows利用者またはSurfaceで、新規インストール、上書き更新、アンインストール、再インストール、表示倍率、Excel、バックアップ・復元を`Surface実機リハーサル手順_0.3.3.md`に従って確認する。
7. 会社規定のコード署名、GitHub公開範囲、配布経路、WebView2・実行制限・ウイルス対策・UNCアクセスを確認する。
...
- `ローカルFAQデータ・Git除外詳細設計書.md` v0.42
