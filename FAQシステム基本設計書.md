...
|---|---|
| 文書名 | FAQシステム基本設計書 |
| 版 | 1.3（FAQ表示フラグ・非表示・DB第2版反映版） |
| 作成日 | 2026-08-08 |
| 上位文書 | `FAQシステム要件定義書.md` v0.6 |
| 対象リリース | MVP（初回リリース） |
...
分類、状態、非表示、更新日時、論理削除状態に索引を設ける。DBスキーマ第2版で表示フラグ3列と検索用索引を追加する。第1版DBおよび第1版DBを含む旧バックアップは、起動・復元時にトランザクション内で第2版へ自動移行し、既存FAQはフラグ未設定・非表示OFFとする。
...
- DB第1版から第2版への起動時移行と、旧バックアップ復元後の移行
- 表・画像・参考URLの保存と再編集
