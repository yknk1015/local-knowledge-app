...
分類の名称変更・移動・削除、FAQの複製・論理削除・復元、画像、複数値項目、関連FAQ、同義語、日本語検索、参考URLの確認付き外部オープン、履歴画面、HTML手順書、JSON入出力は後続の開発単位で実装する。参考URLは確認付き外部オープンが完成するまでクリック遷移を停止する。本節は進捗を示すものであり、後続機能の要件を変更するものではない。
