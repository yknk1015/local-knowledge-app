...
ルートの `Start-App.bat` は隣接する `KnowledgeApp-builds/Start-App.bat` へ委譲する。補助PowerShellがドキュメントフォルダー内の検証済み `KnowledgeApp_Releases/0.7.3/build-20260919-124950-4d58bcc7/standalone/KnowledgeApp.CSharp.exe` を固定指定し、既知のSHA-256一致後に引数なしで起動する。開発binや別版を自動選択しない。`-VerifyOnly` はexeの存在・ハッシュだけを照合し、利用者データを開かない。配布物・C#・UI・データ形式に変更はない。

### 2026-09-19 FAQタブの配置幅修正（C# 0.7.3）

Grid列だけが全幅へ広がり、最大260pxのタブ本体との間に大きな余白が生じていた点を修正する。列幅を`minmax(0, 260px)`、左寄せ、隣接間隔6pxとし、少数時は詰めて並べ、収まらない件数では均等に縮める。タイトル・閉じるボタン・タブの一時状態や遷移処理は変更しない。

### 2026-09-19 単一exeのビルド入口と通常配布（C# 0.7.2）

`npm run build:exe`から`Build-CSharpStandaloneExe.ps1`を呼び、最新UIのビルド、C#依存復元と起動検査用DLLのビルド、既存`New-CSharpStandaloneExe.ps1`の順に実行する。生成した実exeで同梱依存・保存境界・合成DBのログイン表示を検査し、成功時だけ生成先とハッシュを返す。既存`npm run build`はUI専用のままとする。

出力先の省略時はWindowsのドキュメントフォルダー内`KnowledgeApp_Releases/<version>/build-日時-識別子`を新規作成する。明示先も含め、リポジトリ内・既存出力はビルド開始前に拒否する。`standalone/KnowledgeApp.CSharp.exe`だけが実行時の配布対象で、README・manifest・SHA256SUMSは参照用とする。開発用binのexe単体とは区別する。アプリの実行コード・保存形式は変更しない。

### 2026-09-16 起動準備の並行化・単一exe配布（C# 0.7.2）

同梱UIの完全性とWebView2 Runtimeを確認後、毎回新しいブラウザープロファイルで環境を作成する。`EnsureCoreWebView2Async`を開始し、そのネイティブブラウザー準備時間に同期のDB初期化・安全バックアップ・分類カタログ準備を重ねる。DBはUIスレッドで所有し、終了後にバックグラウンド処理がDBを再代入する経路を作らない。開始したブラウザー初期化はDB側の失敗時にも完了を待ち、PIDを既存の終了管理へ渡す。画面は準備中`about:blank`のままとし、DBとブラウザーの両方の準備成功後に既存の設定・CSP・固定ブリッジを設定してログイン画面へ遷移する。

`KnowledgeApp.StartupBenchmark`は名前を限定した内部テスト入口から実際の`MainWindow.OnLoaded`を使用し、TEMP直下のGUID付き合成ルートだけを開く。利用者向けexeの起動引数やWebView APIへ任意保存先・計測機能を追加しない。旧版比較では空DB・Release・ログイン入力可能な状態をそろえ、旧版の永続WebView2プロファイルとC#の毎回新規プロファイル、C#の安全バックアップの差を明記する。

単一exe版は`win-x64`の.NET自己完結発行と`PublishSingleFile`、`IncludeAllContentForSelfExtract`を使用する。既存のUI manifest、SQLite/WebView2のネイティブ依存、自己完結PathProbe、ライセンス案内を発行前にまとめる。初回に.NET標準の利用者別・ビルド別キャッシュへ展開し、`AppContext.BaseDirectory`を使う既存のUI・補助exe解決を維持する。アプリ本体の圧縮は行わず、再起動時のキャッシュを再利用する。FAQと毎起動のブラウザープロファイルはこの配布物キャッシュへ保存しない。

`New-CSharpStandaloneExe.ps1`で作成し、`Test-CSharpStandaloneExe.ps1`が専用の`DOTNET_STARTUP_HOOKS`を子プロセスだけへ設定して、展開後・`App.OnStartup`より前に検査する。実DBを開かず、単一exeの構成、自己完結ランタイム、同梱UI・補助プロセスとデータ混入禁止を確認する。対象PCのWebView2・実行許可・署名条件は別確認とする。配布物と実行結果はリポジトリ外に置く。

保存先・Codex連携先のアプリ領域除外には、`AppContext.BaseDirectory`と`Environment.ProcessPath`の親の両方を使う。前者は展開した実行資源、後者は配布exeの置き場所を保護する。保存先確認の補助プロセスを呼ぶ前にも本体側で文字列だけの重複拒否を行い、ネットワークI/Oの10秒制限は維持する。任意exe検索や親プロセス探索を追加しない。

`CodexLocationService.Read`でも保存済みカスタム連携先を再検査し、exeの移動で重複した場合は所有者ファイルを読む前に`CDX-024`で停止する。未設定の標準先と、標準へ戻した後の正常な再読込は維持する。

単一exeの検査へ`-StartupBenchmarkAssembly`を明示すると、検査hookが専用STAスレッドで合成用起動ハーネスを実行する。配布exeから読み込んだ本体・UI・ランタイムのままログイン画面と安全バックアップを確認し、通常の本番起動へ進まず終了する。これは検査用DLLの機能であり、利用者向けexeの引数・APIへ追加しない。

### 2026-09-06 起動時の白画面・異常終了修正（C# 0.7.1）

`WebResourceRequested`で取得する`CoreWebView2Deferral`は`using`による`Dispose`だけで完了させる。`Dispose`自身が完了通知するため、`finally`での`Complete()`を併用しない。0.7.0では二重完了により`0x8000000E`がWPFへ伝播して異常終了していた。200応答、拒否・欠落の403応答、CSP、認証・パスの境界は維持する。

`KnowledgeApp.StartupCheck`は実際のWPF資源ハンドラーとビルド済みReactを実WebView2で動かし、ログイン画面・CSS・初回読込と2回の再読込・欠落資源403・ブラウザー終了を確認する。DB初期化は実行せず、未認証の応答だけを合成する。従来の純粋なホスト境界試験に加え、実COMイベント処理をCI・配布前で検査する。
...
C#で暗号学的乱数による英数字16文字（32種類・80ビット）の復旧キーを生成し、管理者IDに結び付いたsalt付きArgon2idハッシュを保存する。初回案内、本人の通常パスワード再確認付き発行・再発行、紛失画面、試行制限、5分の復旧限定権限を追加する。パスワード更新・旧キー失効・新キー発行を同一トランザクションで行い、通常・復旧セッションを失効させる。キーは一度だけ表示し、新パスワードで通常ログインする。メール通信は追加しない。 詳細・実装順序・試験は[管理者パスワード復旧 実装計画](管理者パスワード復旧_実装計画.md)を参照する。C# 0.6.0で実装し、下記の旧暫定認証・DB第7版固定の記述を本節で更新する。過去の試験・配布記録は当時の履歴として保持する。実利用環境の更新・受入を自動試験済みとは扱わない。
...
用途別の保存先解決、管理者専用設定、既定先と前回先の分離、C#とプラグインの設定契約共有を計画案とする。Codex連携先の切替とローカル正本の移設は通常の既定フォルダー変更から分ける。複数人共有は、各PCからC#共有サービスへ接続し、ホスト内のDB・画像を扱う構成を第一候補とし、同時編集・サーバー認証・障害対応を別途設計する。
...
| 版 | 1.84（少数FAQタブの間隔調整） |
| 作成日 | 2026-08-08 |
| 上位文書 | `FAQシステム要件定義書.md` v1.59 |
| 対象リリース | MVP（初回リリース） |
| 仮称 | KnowledgeApp（正式名称決定後に置換する） |

### 現在の正式構成（2026-09-06確定）

今回の実装版はC# 0.6.0（.NET 10 / WPF / WebView2 / React / Tiptap / SQLite FTS5 / 利用者単位NSIS）である。C# 0.5.0の利用者による復元完了報告を受けた主系切替を維持し、今回の0.6.0更新の受入とは区別する。以後のバックエンド改修はC#へ実装し、UIを維持する。対象環境等の未実施確認とは区別する。

旧Tauri/Rustの構成説明と段階ごとの切替保留は履歴として残し、現行構成は4.6.13を優先する。移行SQLは`src-csharp/KnowledgeApp.Data/Migrations`から同じ資源名・内容で埋め込む。通常CIはC#・Reactと独立NSISを使用し、Rust/Tauriをビルドしない。NSISコンパイラーはPATHまたは開発用`KNOWLEDGEAPP_NSIS_COMPILER`で明示する。アプリの保存先・権限・UI・DB形式は変更しない。

旧Rust比較は手動起動の`legacy-compatibility.yml`へ分離し、固定コミット`277f38c2a374889d21cb61798ab072dfb16e98be`を別チェックアウトで用いる。ローカルでは`KNOWLEDGEAPP_LEGACY_SOURCE`へ旧`src-tauri`の絶対パスを明示する。これは試験プログラムだけのソース指定であり、本番アプリ・プラグインのデータルート設定ではない。`FinalInteropCheck`の通常実行はC#固定ルートとPowerShellコマンドを検証し、Rust比較は未実行と明示する。`--with-legacy`の場合だけRustバックアップ往復を行う。`CodexInteropCheck`も旧ソースの明示指定を必須とし、未指定/不正パスは失敗させる。合成DB以外は扱わない。

### 文書管理の整理（2026-09-06）

現行の入口をREADMEと[文書整理ガイド](文書整理ガイド.md)、残課題の進行管理を[今後の課題](今後の課題.md)へ分離する。旧版/移行計画37文書は原文を外部保管し、現行文書は削除対象の旧文書を必須手順として参照しない。C#正式切替ガイド・第17段階記録・最終自動検証記録と正式3設計書は現行ツリーに残す。本書の既存節番号・設計履歴は維持する。これは文書配置・参照の整理であり、アプリ構成、UI、コマンド、DB、認証、保存先、配布処理の設計変更ではない。
...
2026-08-16の利用者判断により、対象PCで利用者本人だけがソースを取得して試行する段階を、複数人への正式配布と分離する。一人利用試行では、コード署名・正式配布経路・運用上の承認、Surface再試験、旧版更新・アンインストール・別PC復元・二重起動の広範な手動リハーサル、表示倍率・アクセシビリティの組合せ確認、大容量CSV・バックアップの画面応答と6桁管理IDの実表示、ネットワーク・UNC障害、実環境の資料参照を2026-08-16付で延期する。

延期は機能削除または合格を意味しない。次の再開条件で該当設計・試験を再びリリース条件へ戻す。

| 設計・試験項目 | 再開条件 |
|---|---|
| コード署名・正式配布・公開範囲 | Publicリポジトリは利用者データと外部資料を除外して初回公開する。利用者追加、正式配布、機密情報を含む運用、または利用環境の規定による要求時は公開範囲と配布条件を再確認する |
...
| 実環境のパス・HTTP/HTTPS資料 | 実FAQから外部管理資料を参照する前 |

一人利用試行の開始条件は、対象コミットの固定取得、対象PCでの起動、利用者別ローカルデータルートの確認、通常パスワードと初期管理者緊急認証の確認、緊急認証情報の分離管理、ローカルのフルバックアップ・復元確認、Gitフックと利用者データ混入防止検査である。これらは延期しない。実際の利用環境の規定が署名、申請、保存場所または情報取扱いを要求する場合は、その規定を優先する。

この判断はリリース・検証範囲だけを変更し、画面、Tauri Command、DBスキーマ、保存形式、検索、バックアップ形式およびセキュリティ実装を変更しない。正式リリース判定は延期事項の必要範囲が完了するまで保留する。

### 1.1.3 2026-08-29 次期開発の設計基準

C#移行を必須の最終到達点へ変更する。ただし、本番切替は現行Tauri版に対する非デグレ同等性ゲートの全項目合格を条件とする。制限が残るC#版は試作として分離し、現行版を主系のまま保守する。これにより「C#であること」と「利用できる機能を減らさないこと」を別々に判定する。

先行機能としてFAQ詳細タブを最大10件で実装する。11件目では既存タブを自動破棄せず、利用者に閉じるタブの選択を求める。Outlook履歴は常駐監視せず、利用者の明示操作時だけローカルC#境界で読取・選択・マスクし、選択済み本文だけをCodex向け委譲へ変換する。

2026-08-30にC#バックエンドの第1段階として、認証・利用者権限とSQLite第1～7版の移行基盤を実装した。C#はRust版と同じマイグレーションSQLを埋め込み資源として参照し、初期管理者、Argon2id、通常・緊急認証、利用停止、最後の管理者保護、空パスワード方針、監査担当者補完を同じ契約で扱う。WPF/WebView2からは認証用途に限定した型付きメッセージだけを受け、任意SQL・任意ファイル操作を公開しない。自動試験と手動UI確認が終わるまでは、OS一時フォルダに作成し終了時に破棄する合成DBだけを開き、本番DBへ接続しない。

同日に第2段階として、分類の一覧・追加・更新・移動・並べ替え・削除、同義語CRUD、検索3範囲、NFKC正規化、日本語3文字分解、短語補助、FTS5 trigram候補抽出、項目別重み付け、一致理由、50件ページング、表示対象除外、明示検索時の履歴記録をC#へ移植した。現行React画面と型付きAPI契約を共用し、合成DB自動試験では1～5階層、循環・6階層・同一親同名・使用中削除の拒否、同義語展開、下書き条件、非表示・削除済み・統合済み除外、SQL入力の非実行を確認する。利用者手動試験が合格するまで`categories_search`ゲートは未合格とし、本番DBは開かない。

分類・検索段階の利用者手動試験26項目は2026-08-30に合格した。うち6階層追加と循環移動の2項目は、当初の手順で想定した`CAT-003`・`CAT-006`の保存エラーより前に、React画面が無効な親候補をプルダウンから除外した。これは6.4で定めた画面側と保存側の二重防止に一致するため、画面試験は「無効な候補を選択できないこと」、C#合成DB自動試験は「直接要求でも固定エラーで拒否すること」として合格扱いに訂正する。`categories_search`ゲートを合格とし、次のFAQ詳細表示・タブ用C# API段階へ進んだ。本番DBは引き続き開かない。

同日に第3段階として、現行ReactのFAQ詳細画面が使用する`get_article`と`record_article_view`をC#へ移植した。C#はタイトル、分類名、概要、Tiptap JSON、プレーン本文、状態、重要度、表示フラグ、作成・更新者、削除・統合情報、旧検索情報、タグ、関連FAQを既存SQLite形式から型付きで返す。FAQ詳細の取得だけでは履歴を増やさず、Reactが詳細取得とタブ上限確認に成功した後の明示要求でだけ閲覧履歴を1件記録し、検索結果から開いた場合は既存検索ログIDを関連付ける。合成DB自動試験では異なる10件の詳細読取、関連FAQ・統合・削除表示、Tiptap JSON、SQL文字列の非実行、検索ログID検証を確認した。利用者手動試験18項目も同日にすべて合格し、現行React/Tiptapの詳細表示、関連FAQ、最大10タブ、重複防止、11件目拒否、一覧・FAQ別スクロール、タブ終了、再起動時の一時状態破棄を確認したため、`article_view_tabs`ゲートを合格とする。画像・表・参考URL・コピー用テキストおよび添付パス解決は後続の`rich_text_attachments`ゲートで判定する。

同日に第4段階として、現行ReactのFAQ編集4区分とFAQ管理一覧を維持したまま、`list_tags`、`save_article`、`duplicate_article`、`list_articles_for_management`、`delete_article`、`restore_article`をC#へ移植した。新規作成・更新・複製・論理削除・復元はすべてパラメーター化SQLとトランザクションで処理し、管理ID、作成者・更新者、表示フラグ、タグ、検索文書、FTS5索引を同時に更新する。現行編集画面から変更しない旧検索情報と既存関連FAQは通常更新で削除せず、複製時は旧検索情報・関連FAQ・タグを独立FAQへ引き継ぐ。Tiptap本文は見出し、太字・斜体、リスト、表、HTTP/HTTPS参考URL、コピー用テキストの許可構造をC#側でも検証する。合成DB自動試験に加えて現行React画面による利用者手動試験23項目がすべて合格したため、`article_editing`ゲートを合格とする。本番DBは引き続き開かない。

同日に第5段階として、`rich_text_attachments`ゲートのC#実装を行った。画像選択はWPFのファイルダイアログ、クリップボード画像はWebView2からのBase64型付き要求として受け、C#側で10MB上限、PNG・JPEG・WebP・GIFのマジックバイト、サイズ、SHA-256を再検証する。保存前は合成データルートの`temp/staged-article-images`へUUID名で一時保存し、FAQ保存成功時だけ`attachments/articles/{articleId}`へ確定する。DB失敗時は新規確定ファイルを戻し、本文から外した既存画像はDB保存成功後だけ削除する。添付付き複製では本文中の`attachmentId`を新UUIDへ置換して別ファイルを作成し、論理削除・復元では画像を保持する。WebView2へは確定画像と一時画像の各専用仮想ホストだけを公開し、DB・設定・バックアップは公開しない。クリック可能な参考URLはReact確認画面の後にC#側でもHTTP/HTTPS、接続先、認証情報なし、2,048文字以下を再検証して既定ブラウザーへ渡す。コピー用テキストは読取・自動実行を行わず、C#のUIスレッドでプレーンテキストを書き込む。合成DB自動試験と利用者手動試験がすべて合格したため、`rich_text_attachments`ゲートを合格とする。

同日に第6段階として、検索・閲覧履歴画面が使用する`list_search_logs`、`list_view_logs`、`delete_history`をC#へ移植した。履歴一覧は認証済み利用者だけに許可し、検索文をNFKC正規化してLIKE予約文字を無害化し、検索履歴は検索文・日付・0件のみ、閲覧履歴はFAQタイトル・元の検索文・日付で絞り込む。日付は`YYYY-MM-DD`をUTC日境界へ変換し、開始日が終了日を超える入力を`LOG-002`で拒否する。一覧は日時と内部IDの降順で50件ずつ返し、範囲外ページは最終ページへ補正する。削除対象は検索履歴または閲覧履歴の固定値だけを受け、期間削除と全件削除をトランザクションで実行する。検索履歴を削除しても、外部キーの`ON DELETE SET NULL`により閲覧履歴自体を保持し、画面では元検索なしとして表示する。合成DB自動試験は認証境界、54件ページング、NFKC・日付・0件絞り込み、SQL文字列の非実行、期間・全件削除に合格した。利用者手動試験20項目もすべて合格し、検索・0件検索・閲覧の3タブ、文字列・日付絞り込み、削除対象確認、検索履歴削除後の閲覧履歴保持、期間・全件削除、再起動時の合成履歴破棄を確認したため、`history`ゲートを合格とする。本番DBは引き続き開かない。

同日に第7段階として、現行ReactのFAQ管理、CSVインポート、JSON入出力画面が使用する`export_faq_csv`、`inspect_faq_csv`、`import_faq_csv`、`export_json`、`inspect_json`、`import_json`をC#へ移植した。TauriのファイルダイアログはC#ホストの場合だけWPFの固定CSV／JSONダイアログへ切り替え、選択後の拡張子補正、プレビュー、確認、件数表示は同じReact画面を共用する。C#側は絶対パス、固定複合拡張子、既存親フォルダ、Git管理外を再検証し、任意ファイル操作を公開しない。CSVは固定17列、UTF-8 BOM・CRLF、RFC 4180、数式無害化、内部UUID非出力、本文ハッシュ、旧版上書き警告を維持する。JSONは形式版、必須・未知項目、UUID・管理ID・日時・5階層・重複・許可Tiptap・関係整合性・既存DB衝突を検査し、画像参照と画像本体、利用者・認証、履歴、設定を除外する。取込前にはDBスナップショット、添付、旧互換`manuals`、設定を現行manifest付き`.faqbackup`へ格納してSHA-256再検証し、取込は単一SQLiteトランザクションで行う。合成DB自動試験は構造化本文保持、本文段落置換、新規CSV行、数式無害化、別DBへのJSON移行、更新、危険ノード・未知項目・形式版・Git配下拒否、SHA再検査、安全バックアップに合格した。利用者手動試験33項目も、再起動時の合成タグIDを固定した後の再試験を含めてすべて合格したため、`csv_json`ゲートを合格とする。本番DBは後続ゲート完了まで引き続き開かない。

続く第8段階として、現行React設定画面の`get_system_info`、`get_settings`、`save_settings`、`get_backup_overview`、`create_full_backup`、`inspect_backup`、`restore_backup`をC#へ移植した。C#ホストでは保存先と復元元の選択だけをWPFの`.faqbackup`専用ダイアログへ切り替え、端末情報、配色・タイトル分類・マスコット表示、概要、進行中表示、同名上書き確認、復元前プレビュー、最終確認、結果表示は既存Reactを共用する。C#側は管理者権限、絶対パス、固定拡張子、Windows名、既存親フォルダ、Git管理外、書込可否、空き容量を再検証する。SQLite Backup APIで作ったDBスナップショット、管理添付、旧版互換`manuals`、設定をformat version 1のZIP互換コンテナへ格納し、manifestのファイル数、サイズ、SHA-256、DB版、外部キーを検証してから部分ファイルを確定する。復元は専用ステージへ検証展開し、現在状態を`safety-backups`へ退避した後、DBと管理フォルダを入れ替える。失敗時はDB・フォルダを戻し、成功時はセッションを破棄する。自動試験は画像、利用者認証、分類、配色・タイトル分類表示・マスコット表示、旧版互換ファイルを含む往復に合格した。利用者手動試験が終わるまでは`backup_restore`ゲートを未合格とし、本番DBは開かない。

2026-09-05に利用者から、タイムアウト不合格項目の再試験と残りのフルバックアップ未実施項目がすべて合格したとの報告を受領し、第8段階の`backup_restore`ゲートを合格とした。続く第9段階ではCodex提案・明示委譲・履歴・統合関係をC#へ移植し、合成DB・合成ファイルの専用自動試験45シナリオと利用者U1～U7の画面試験に合格した。実プラグインとC#本番固定ルートの往復は未確認のため、`codex_bridge`全体は未合格を維持する。第10段階のタグマスター・ホスト／ファイル安全境界と後続ゲートの確認を進め、本番DB未接続・現行Tauri版主系を維持する。
...
テーマは「日常から仕事まで、セキュアにFAQナレッジを活用できる」とする。利用場所や題材で機能を分けず、利用者別ローカル保存、認証・権限、外部URL確認、Codexへの明示委譲、バックアップ・復元を共通の安全境界とする。

実際のFAQ・データベース・パスワードなどの利用者データや認証情報は、公開・非公開を問わずGitHubを含むGitリポジトリへ絶対にコミットしない。派生するバックアップ・エクスポート・履歴・画像・ログ・委譲ファイル、APIキー・トークン・秘密鍵・緊急認証情報も除外し、文書やソースへの転記も禁止する。既存のリポジトリ外保存、Git除外、コミット前・CI・配布物検査を維持し、ファイル名による混入検査だけで本文中の秘密をすべて検出できるとはみなさない。詳細はデータ分離詳細設計書に従う。

| 項目 | 設計内容 |
|---|---|
| 開発環境 | Windows 11 64ビットの開発用PC |
| 利用目的 | 日常の疑問から業務知識まで、FAQを安全に作成・検索・編集する。 |
| 題材 | 学習、生成AI活用、PC設定、ネットワーク、プログラム修正など。利用場所で区分しない。 |
| MVP利用者 | 1名 |
| 配布 | 単一exeを取得・コピーして起動。NSISとフォルダー型ZIPは補助方式 |
...
- FAQの題材は日常の疑問、学習、生成AIの活用事例などを含み、利用場所で限定しない。
- PCトラブル、PC設定、ネットワーク、プログラム修正などの業務知識も同じ機能で扱う。
...
メール履歴は機密情報を含み得るため、検索対象や常時収集元へ追加しない。利用者がその都度「メール履歴を確認する」を実行した場合だけ、C#のローカルメール取込サービスを起動する。初期版は`.msg`を対象にし、`.pst`はランダムアクセス、暗号化、破損、文字コード、巨大ファイル、Outlook版差および利用環境の規定を別ゲートで確認してから追加する。

```text
利用者の明示操作
  → 指定フォルダを読取専用で1回走査
  → ローカル一覧・本文プレビュー
  → 利用者が対象を選択し、不要箇所を削除またはマスク
  → mail-delegationsへ選択済み本文だけを原子的に保存
  → 委譲番号をCodexへ明示
  → 確認待ちFAQ提案
  → 利用者の承認後だけ新規下書き
```

走査処理はフォルダウォッチャー、起動時処理、スケジューラーを持たない。原本と添付をアプリデータへコピーせず、委譲JSONに原本の絶対パス、Outlookプロファイル名、未選択メール、添付、履歴を含めない。Codexプラグインは委譲番号に一致する単一ファイルだけを開き、指定フォルダやメール原本を探索しない。提案形式は既存の`create`を使い、自動公開・自動削除を行わない。

`.msg`はまず`OpenMcdf`で複合ファイルのルートと宛先ストレージにある標準MAPI項目（件名、送信者、宛先、日時、プレーンテキスト本文または安全に通常テキスト化したHTML本文）を読取専用で取得する。添付ストレージへ入らず、添付データ、任意HTML、スクリプトを表示・実行しない。本文が標準MAPI項目から取得できない場合だけ`MsgReader`で互換解析し、圧縮RTFなどの本文取得を試みる。一部項目だけが読めない場合は当該項目を空欄にして利用者へ警告し、全項目を安全に取得できない場合だけ除外する。警告は原本文、アドレス、絶対パス、例外メッセージを含めず、`MAIL-READ-PARTIAL`、`MAIL-READ-LOCKED`、`MAIL-READ-ENCODING`、`MAIL-READ-FORMAT`、`MAIL-READ-UNSUPPORTED`の固定原因コードで案内する。

## 3. 配置・配布設計

### 3.1 配布・導入方式

- 通常利用はインストール不要の単一exeを採用し、専用ローカルフォルダーへコピーして直接起動する。
- 必要な利用環境向けにNSIS形式の利用者単位インストーラーも維持し、スタートメニュー・デスクトップのショートカットを作成できるようにする。
- 単一exeの更新時はアプリ終了後にexeを交換する。NSIS版はインストーラーで上書き更新する。
- exeの削除・アンインストールで利用者データは削除しない。
- ポータブルZIP版は試用・障害切り分け用の補助配布とし、通常運用の第一候補にはしない。
- C# 0.7.2の単一exeは自己完結.NETを内蔵し、WebView2は対象PCの既存Runtimeを使う。データ保存先はインストーラー版と共通とし、起動中は両方式の同時利用を既存の起動排他で防ぐ。
...
| `codex-bridge` | FAQ本文を含まない分類カタログ、利用者が明示選択したFAQの委譲、および明示走査・選択・マスク済みメール本文の委譲。メール原本、原本パス、添付は置かない。 | × |
| `codex-inbox` | 利用者が確認する前のCodex FAQ提案 | × |

インストール先やGitリポジトリには利用者データを書き込まない。通常版はRust側でTauriの`appLocalDataDir`を解決し、この固定パスだけをDB、添付、設定の基準にする。パス解決に失敗した場合、現在の作業ディレクトリへ代替保存せず起動を停止する。アプリ外で管理する外部資料はデータルートへ複製しない。ポータブル版の保存方式は実装時に本方針を維持して別途定義する。
...
| Article View | 最大10件のFAQ詳細タブ、既存互換の関連FAQ表示、外部資料の参照先、参考URL、コピー用テキスト枠 |
...
### 4.6 C#移行中の二重構成と切替ゲート

移行中は現行Tauri版とC#試作版を同じリポジトリで管理するが、利用者データの同時書込みは許可しない。C#試作版は最初にWPF＋WebView2で既存`dist`を表示する互換ホストとし、React＋TypeScript＋Tiptapの画面資産を再利用する。バックエンド機能は用途別の型付きメッセージとして段階移植し、任意SQL、任意シェル、任意ファイル操作を公開しない。

```text
現行主系: React UI → Tauri invoke → Rust service → SQLite/利用者データ
移行試作: 同一React UI → WebView2 message → C# service → SQLite/利用者データ
                                          ↓
                         自動同等性レポート＋Windows手動UI判定
                                          ↓
                    全必須項目合格時だけ本番切替を許可
```

同等性レポートは少なくとも、UI読込、認証、分類、検索、FAQ表示、FAQ編集、Tiptap、添付、履歴、CSV、JSON、バックアップ、Codex、データルート、SQLite/FTS5、単一起動、配布、手動UI確認を持つ。必須項目の論理積だけで`cutoverAllowed`を算出し、未実装項目を手動で合格扱いにできない。未合格がある間は`deferred_until_parity`として本番切替の優先度を下げ、現行Tauri版を主系とする。

C#試作版は現行と同じ`%LOCALAPPDATA%\jp.local.webknowledgesystem`を最終互換先とするが、書込系が完成するまでは既存DBを開かない。読取試験でも最初はOS一時フォルダ内の合成コピーを使用し、現行版とC#版を同じ本番DBへ同時接続しない。

C#認証段階ではOS一時フォルダ直下の`knowledgeapp-csharp-auth-{UUID}`、分類・検索・FAQ詳細・FAQ編集段階では`knowledgeapp-csharp-search-{UUID}`へ起動ごとの合成DBを新規作成し、ウィンドウ終了時に接頭辞・一時フォルダ直下であることを再検証してから削除する。C#データ層は一時フォルダ外のパスを拒否し、SQLiteの外部キー、WAL、busy timeout、パラメーター化SQL、トランザクションを使用する。既存合成DBが第1～6版の場合は`quick_check`と必須表を検証し、SQLite Backup APIによる移行前スナップショットを作成・再検証してから第7版まで移行する。破損DB、未知の将来版、バックアップ失敗は書込み前または移行前に拒否する。本番データルートを開く処理は、FAQ CRUD・FTS5・添付・設定・履歴・バックアップの各同等性ゲートが合格する段階まで実装しない。

#### 4.6.1 C#ブリッジの待機期限と終了保護

`src/api/csharpBridge.ts`は要求IDごとに待機を管理し、固定コマンド名の完全一致で次の方針を選ぶ。画面から任意の期限を渡すAPIや接頭辞一致による除外は設けない。

| 対象 | 固定コマンド | 待機方針 |
|---|---|---|
| ファイル選択7件 | `select_article_image`、`select_faq_csv_export_path`、`select_faq_csv_import_path`、`select_json_export_path`、`select_json_import_path`、`select_full_backup_destination`、`select_restore_backup_source` | 30秒タイマーを設けず、選択・キャンセル・検証失敗の実応答を待つ |
| 長時間入出力9件 | `create_full_backup`、`inspect_backup`、`restore_backup`、`export_faq_csv`、`inspect_faq_csv`、`import_faq_csv`、`export_json`、`inspect_json`、`import_json` | 画面側だけの期限切れでは取消不能なため、C#の実応答まで既存の処理中表示を維持する |
| Codex状態更新7件 | `list_codex_proposals`、`accept_codex_proposal`、`reject_codex_proposal`、`reopen_rejected_codex_proposal`、`create_codex_delegation`、`mark_codex_merge_sources`、`clear_article_merge` | カタログ・履歴・委譲ファイル・FAQ・統合関係の処理を誤って再実行しないよう、C#の実応答を待つ |
| タグ保存1件 | `save_tag` | 多数の使用中FAQの索引更新を画面側だけで失敗扱いにせず、C#の実応答を待つ |
| その他 | 上記以外の通常要求 | 従来の30秒期限で`SYS-001`を返す |

応答時は一致した要求だけを解決・除去し、設定されているタイマーを解放する。不明ID、重複応答、期限切れ後の応答は無視する。送信時に例外が発生した場合も待機とタイマーを解放し、固定の`SYS-001`で案内する。要求内容や例外詳細のログ出力、自動再試行、C#処理の強制取消は追加しない。

WPFホストの`HostOperationPolicy`は長時間入出力9件、Codex状態更新7件とタグ保存1件の計17件を終了保護対象とし、開始前に実行数を増やし、成功・失敗のいずれでも`finally`で減らす。実行中の終了要求を固定の日本語案内で拒否し、処理中のDB・合成データルート破棄を防ぐ。専用ファイルダイアログは既存のモーダル表示を維持する。待機対象のホストが応答不能となった場合は経過時間だけで成功・失敗を推測しない。読取専用の`get_codex_merge_publication_context`と通常の`delete_tag`は30秒期限を維持する。

合成メッセージによる遅延成功・取消・エラー、通常30秒期限、同時要求ID分離、タイマー解放、固定対象の範囲を自動試験する。C#側は固定17件と対象外の終了保護判定、および既存の合成DB入出力・復元試験を実施する。45秒待機の実ダイアログ操作は専用Excelによる利用者再試験で合格した。以後の実画面確認はMarkdown記録を基本とし、未実施の画面試験を自動試験の合格で代替しない。

#### 4.6.2 C# Codex連携層

`CodexProposalFiles`は既存合成DBから決定した固定ルートの`codex-bridge/categories.json`、`codex-bridge/delegations`、`codex-inbox`だけを扱う。`CodexProposalService`は認証済み利用者に限定して8契約を公開し、`KnowledgeDatabase.Codex`は既存DB第7版の受付・履歴・FAQ・分類・索引・統合関係をパラメーター化SQLとトランザクションで処理する。Dispatcherは現行Reactの引数と戻り値を維持し、任意パス・SQLを追加しない。

一覧時の受付記録とカタログ更新、承認、却下、再検討、委譲作成、統合済み化、解除はサービス内で直列化する。起動時、分類変更後、復元後はホストの用途限定フックで分類カタログを再生成し、一覧時にも最新化する。カタログ更新失敗によって確定済みの分類変更や復元を再実行させず、一覧更新で再確認できる。DBの保存形式、管理ID採番、画像管理、履歴保持、バックアップ対象は変更しない。

`CodexSyntheticUiFixture.Seed(database)`は、新規作成された試作専用一時ルートに限り、固定の合成元FAQに対応する合成委譲と新規・修正・統合の3提案を準備する。既存ルートや復元後へ再投入せず、認証・提案履歴受付・元FAQの変更を行わない。実画面試験はこれらの合成案を使う。現行専用プラグインは本番固定ルートを使うため、この段階では試作ルートへ接続せず、試作の依頼文を本番向けプラグインへ送らない。新しい試作exeは`src-csharp/KnowledgeApp.CSharp/bin/codex-migration-20260905/KnowledgeApp.CSharp.exe`へ分離し、起動中の旧版を上書きしない。

#### 4.6.3 Codex画面受入とタグマスター契約

2026-09-05にU1～U7すべての利用者合格報告を受領した。第9段階の合成提案による画面操作は受入完了とする。既存専用プラグインとC#本番固定ルートの実往復は未実施であり、`codex_bridge`の残条件として明示する。

第10段階で`save_tag`・`delete_tag`を既存Dispatcherから`ArticleEditingService`とタグ専用Repositoryへ接続する。タグを選択する既存画面・`list_tags`は変更しない。保存はNFKC・英字大小・空白正規化による同名検証、ID維持、1～100文字・改行拒否、変更先存在を検証する。名称変更と使用中FAQの検索文書・FTS5更新を同じトランザクションで行い、FAQ本文・旧検索情報・状態・監査列は維持する。削除は使用件数0のタグだけに限定し、認証済み一般利用者にも既存版と同じ範囲で許可する。タグ管理のUI受入は必須`tag_master`ゲートで追跡する。

`save_tag`は使用中FAQ数に応じて索引更新が長時間となるため、4.6.1の実応答待機と終了保護へ追加する。`delete_tag`は通常の30秒期限を変更しない。

#### 4.6.4 C# WebViewの信頼境界

正規ドキュメントは`https://app.knowledge.local/index.html`とそのHashRouterの画面遷移だけとする。要求の送信元と現在のドキュメントを照合し、JSON要求の型・長さ・受付IDと重複項目を検査する。ドキュメントの世代を保持し、非同期処理開始後に再読込・遷移した場合は古い要求への結果を新しいページへ返さない。通常要求と固定長時間処理の待機・終了保護は4.6.1を維持する。

アプリ内の外部遷移、子フレーム、新しいウィンドウ、ダウンロードと不要なWeb権限は拒否する。開発者ツール・汎用ホストオブジェクトを公開しない。起動時に注入するブリッジ識別情報も正規origin・パスに限定する。参考URLは既存の確認画面と`open_external_url`のHTTP/HTTPS検証を通った場合だけ、既定ブラウザーへ委譲する。

仮想ホストのフォルダマッピングに依存せず、C#の資源応答処理でアプリの静的資産、固定UUID配置の確定画像、一時画像だけを許可する。DB・設定・バックアップ・添付メタデータを返さない。正規HTML応答へCSPと`nosniff`を付与し、外部通信・フレーム・オブジェクト・任意フォーム送信を拒否する。ビルド済みJS/CSS、遅延画面読込、インラインスタイル、マスコット、管理画像と必要な一時画像表示を許可し、React画面のレイアウトは変更しない。UIの起点は実行ファイル側から解決し、任意のカレントディレクトリで別の`dist`を選ばない。

WebView2環境の利用者データフォルダを合成ルートの`temp/webview2-profile`へ明示指定し、exe隣にプロファイルやキャッシュを生成する既定動作を使わない。`CreateAsync`後の`environment.UserDataFolder`を期待パスとI/Oなしで正規化比較し、不一致の場合は`EnsureCoreWebView2Async`前に固定`SYS-001`で停止する。未知のプロファイル、環境変数、レジストリは読取・変更しない。終了時のプロファイル破棄は4.6.6に従い、以前の`bin`配下に残る既存プロファイルを自動探索・削除しない。

実装根拠はMicrosoftの[WebView2セキュリティ指針](https://learn.microsoft.com/en-us/microsoft-edge/webview2/concepts/security)と[WebResourceRequestedの扱い](https://learn.microsoft.com/en-us/microsoft-edge/webview2/how-to/webresourcerequested)とする。合成URL・要求・資産で許可と拒否を自動試験し、正規UIの動作は別に確認する。

#### 4.6.5 C#ファイルの実体境界

`FileSystemBoundary`へ合成ルートとファイル境界の共通検証を集約する。許可接頭辞だけでなくUUIDとOS一時フォルダ直下を確認し、ルートを含む既存祖先・対象の再解析点を拒否する。SQLite接続前、添付の生成・読取・削除、バックアップの収集・入出力・復元、CSV/JSON入出力の各境界で再検証する。Git外のリンクからGit内へ到達するパスも拒否し、デバイスパス・代替データストリームを通常ファイルとして扱わない。通常のUNCバックアップは引き続き許可する。

`ValidatePathSyntax`は通常UNCを含むパス文字列の構文検証だけを行う。保存済みバックアップ先の表示・設定取得では`Directory.Exists`などによる自動アクセスを行わず、実際の作成・検査・復元操作時だけ実体・アクセス可否を検証する。構文上の許可とネットワーク共有への実アクセスを分離する。

一時画像の期限切れ整理は既知のUUID生成名だけに限定し、未知ファイルを削除しない。合成ルートの終了時破棄は再解析点を辿らない処理を共用し、対象を安全に検証できない場合は破棄せず残す。これらは異常なリンク配置を拒否する境界であり、同じOS利用者による悪意ある同時ファイル差替えを完全に隔離する仕組みとは位置付けない。DB版、バックアップ形式、利用者の正規保存先は変更しない。

第10段階の自動試験では、タグC#合成24シナリオ、FileBoundaryの合成junction、実Dispatcher配線を含むDataCheck全体、SecurityCheckの既存98シナリオとCodexCheck再試験45シナリオが合格した。終了制御32件とプロファイル照合12件を追加したSecurityCheck計142件もすべて合格した。2026-09-05に利用者からT1～T8全項目合格の報告を受領し、`tag_master`を合格とした。本番固定ルート・ネットワーク・配布環境の同等性確認は未実施で、安全境界のゲート全体は未合格を維持する。T2の旧名検索は共通部分「合成移行」による一致として受容し、手順書の旧名称0件という期待値を訂正する。日本語・英数字分解と部分一致の検索設計は変更せず、完全一致化は後日相談とする。

#### 4.6.6 C#試作の終了制御

`HostShutdownPolicy`で終了条件を分離する。`OnClosing`では4.6.1の固定17処理を先に検査し、実行中なら終了を拒否する。終了開始後は要求を一時保留して二重終了を抑止し、初期化の完了と`BrowserProcessExited`を合計最大5秒まで待つ。`Browser.Dispose`だけでプロファイルのロック解除を推測しない。待機後に`DB.Dispose`を実行し、ブラウザープロセス終了と固定合成ルートの安全性を確認できた場合だけルートを削除し、最後の`Close`を通す。

初期化・プロセス終了の待機期限超過、再解析点やファイルロック等による削除拒否では、一時データを残した旨を固定文言で通知する。プロセスkill、強制削除、自動再試行、他インスタンスや旧試作の終了・削除を行わない。残置パスや内部例外を警告へ展開しない。

#### 4.6.7 第11段階の単一起動と同梱UI

C#の`App.OnStartup`で、`MainWindow`、合成DB、WebViewの作成より前にOS管理の排他を取得する。試作専用の固定製品識別子とWindows利用者の識別子を使用し、exeの場所やビルド日時では排他を分けない。既存Tauri版・第10段階以前の試作はこの仕組みを持たないため、相互排他の合格とは扱わない。本番DBへ接続しないことで同一DBへの二重書込みを避ける。

第2起動は現在のWindows利用者だけが使えるローカルIPCで、固定の画面表示要求だけを送る。任意引数・カレントディレクトリ・ファイルパス・SQLは送らない。受信側はUIスレッドで最小化を解除して表示・フォーカスし、終了開始後は表示要求を拒否する。起動中や応答不能時には最大5秒の待機と固定の日本語案内を用い、既存プロセスの終了・kill・強制ロック解除を行わない。排他は非同期の終了処理中も保持し、アプリ終了で解放する。異常終了後の恒久的なロックファイルを作らない。

`UiBundleLocator`は`AppContext.BaseDirectory/ui`だけを起点とし、同梱画面資産の配置・内容を検証する。開発時もUIビルド後のC#ビルドで同じ配置を作り、祖先の`dist`や任意カレントディレクトリへフォールバックしない。ホストの資源応答・CSP・画像境界は4.6.4を維持する。同梱UIの欠落・不正配置やWebView2未導入は合成DB接続前に検出し、利用者向けの固定説明で停止する。

検証用パッケージは新しい出力先へ必要な実行物・依存物・UIだけを組み立て、最終ツリーの許可リストとハッシュで検査する。既存出力を再利用・上書きせず、DB、WAL/SHM、委譲・提案、エクスポート、添付、ログ、ブラウザープロファイルを含めない。実行環境の前提と依存物の情報を添付し、利用者による配置変更後の起動を確認する。フォルダーパッケージの合格だけではNSIS・正式配布・更新・再導入の合格にしない。`single_instance_distribution`ゲート全体は後続確認まで未合格を維持する。

`New-CSharpTrialPackage.ps1`は新しい`bin/pkg-{ランダム12桁}`でpublishし、固定の実行DLL・Windows x64ネイティブDLL・翻訳資産の許可リストと同梱UIだけを`app`へ組み立てる。未知の依存DLLも黙って省かず拒否する。依存宣言・ローカルにあるライセンス文、実行環境の説明と整合性マニフェストを追加し、ZIPを作成する。.NET 10 Desktop Runtime x64とWebView2を前提とするframework-dependentな試作で、自動導入は行わない。ファイル集合だけでなく空フォルダー・再解析点も検査し、元のReactビルド全30ファイルとのSHA-256一致を確認した。単一起動37件・同梱UI13件・パッケージ9件は合格し、リポジトリ外の実画面でログイン前の表示と別配置からの最小化復元・1ウィンドウ／1プロセス維持を確認した。2026-09-05にログイン後のP1～P4も利用者全合格を受領した。本番・配布の残条件は未実施として分離する。

#### 4.6.8 第12段階：永続検証ルートとブラウザー一時領域の分離

第11段階のP1～P4は2026-09-05に利用者から全合格を受領した。第12段階も同じ試作製品識別子の単一起動をDB接続前に取得し、別配置・第11段階の起動中画面を自動終了しない。UI全体の同梱検証とWebView2利用可否の確認後、引数なしの`KnowledgeDatabase.OpenRehearsal()`で固定検証ルートを開く。`RehearsalDataRoot.FixedPath`は`Environment.SpecialFolder.LocalApplicationData`と固定名`jp.local.webknowledgesystem.csharp-rehearsal`から決定する。本番パス・任意パスの指定を公開しない。

初回は存在しないか空のルートだけに初期化中マーカーを排他的に作り、DB第1～7版SQL・初期管理者・整合検査の成功後に完了マーカーへ原子的に確定する。FAQ・分類・タグ・提案のフィクスチャーは呼ばない。既存ルートでは完了マーカー、DB実体、現行版・整合性を検証してから継続利用し、初期管理者の再投入やFAQ監査の自動補完をしない。未完初期化、未知の非空ルート、DB欠落・破損・旧版・将来版は固定`DB-001`で停止し、元ファイルを保持する。既存の`OpenSynthetic`による第1～7版移行自動試験は維持し、永続ルートの旧版起動時移行受入とは区別する。

`FileSystemBoundary.ValidateManagedDataRoot`は従来の厳格なOS一時フォルダー直下合成ルートか、固定検証ルートだけを受け付ける。添付・CSV/JSON・バックアップ・復元の管理元検証をこの狭い境界へ接続する。`ValidateSyntheticRoot`と`DeleteSyntheticRoot`の削除許可は拡張しない。再解析点・デバイス・ADS・通常ローカル以外のルートを拒否し、サービス内の管理パス照合と再検証を維持する。テスト用の根差替えはinternalで厳格な一時合成ルートだけへ限定し、アプリの公開API・設定・起動引数へ渡さない。

`TemporaryBrowserStorage`は引数なしで起動ごとの`%TEMP%/knowledgeapp-csharp-search-{UUID}/temp/webview2-profile`を確保し、その生成ルートだけを保持・整理する。永続DBルートを受け取るコンストラクターや削除引数を設けない。`AuthenticationService`は起動ごとに新規作成し、ブラウザープロファイルも使い回さない。よってログイン・Web Storage・タブ・スクロール・未保存編集は引き継がず、DBへ保存した表示設定等だけを再読込する。4.6.6の固定17処理終了ガードと最大5秒待機を維持し、ブラウザー解放確認後だけ今回の一時ルートを削除する。期限超過時は一時領域の残置を通知し、永続ルートの削除、プロセスkill、別プロファイルの探索を行わない。

既存のフルバックアップと明示復元は同じ管理データ・形式・安全退避・ログアウトを維持する。検証環境の初期化マーカーとブラウザープロファイルはバックアップ対象へ加えず、復元によってマーカーを消去・再生成しない。パッケージのモードと説明を継続保存の検証用へ更新し、FAQ・DB・画像・プロファイル・マーカーを同梱しない。実プラグインの接続先切替、NSIS、正式配布、本番コピー比較は後続ゲートへ残す。

メール画面も既定の本番ルートを使う`MailDelegationWriter()`を直接生成せず、`RehearsalMailDelegation`から固定検証ルートと`codex-bridge/mail-delegations`の実体を事前検証したwriterを注入する。ホスト初期化完了前はメール画面を開かない。明示走査・選択・マスク・最終確認・破棄を維持するが、実プラグインが未接続の検証版では依頼文のクリップボードコピーと送信案内を行わず、専用領域への保存と送信不可を表示する。これは試作専用の制限であり、メール委譲の本番非デグレ要件や既存ライブラリーの本番用契約は変更しない。

第12段階の永続検証環境への復元は現行DB版で有効管理者を持つバックアップに限定する。旧版バックアップからの復元・移行と本番コピー比較は後続ゲートで確認し、この段階の合格に含めない。

#### 4.6.9 第13段階：旧版バックアップのステージ移行

2026-09-05に第12段階R1～R4の利用者全合格を受領した。第13段階は既存の`BackupService`とReactの型付き契約を維持し、検査用コピーと復元用ステージのDBへ`PrepareBackupSnapshotForRestore`を適用する。manifestの形式・元DB版・件数・管理ファイルのサイズとSHA-256を検証した後、移行履歴の連続性・正規スキーマ・外部キーを検査し、同梱のRust第1～7版SQLの未適用分だけを作業用DBへ適用する。プレビューには元のDB版を返し、元アーカイブは書き換えない。

第1～5版は認証テーブルと監査列がまだない正規構成だけを受け付け、移行後に初期管理者と監査初期値を用意する。第6版は既存利用者があればID・hash・権限・監査値を維持し、有効管理者を要求する。利用者0件かつ監査参照がすべてNULLの第6版だけは、Rustの移行前安全バックアップに存在する状態として初期管理者を作成する。第7版の管理者欠落は拒否する。保存済みhashは既存Argon2id認証器の対応形式だけを検査し、パスワードの再ハッシュ・リセットをしない。

第6版で利用者が既に存在し、FAQ監査値にNULLが残る場合は、固定初期管理者IDの実在を確認してコピー上だけで既存`COALESCE`補完を行う。これはRustの初回移行処理の完了に相当し、非NULL監査値と既存利用者・hashは保持する。固定初期管理者がないNULL監査の第6版、およびNULL監査の第7版は`BK-006`で拒否する。検査合格後に詳細取得の利用者JOINでFAQが消える状態を許容しない。

移行後に現行版・必須テーブル・外部キー・有効管理者、分類5階層・循環、許可リッチテキストとFAQごとの添付参照、添付情報とmanifest、既知の表示設定を検査する。未知の旧設定と互換ファイルは不透明データとして保持し、参照先の外部資料へアクセスしない。本文JSON・旧検索情報・検索文書・FTSを再生成せず、第7版の管理ID採番は元SQLに委ねる。移行前後の検査失敗は`BK-006`で拒否する。

復元専用の本文検査は通常編集の10万文字・1万ノード・構造深さ32の量的制限を適用せず、安全なノード・属性・リンク・管理画像の規則だけを共用する。保存JSONの読込はRustのJSON既定境界に合わせた最大深さ128とし、FAQ詳細取得も同じ境界で読む。ホスト応答はDTO外側の余裕を含め最大深さ144で直列化する。受信要求の上限と通常編集の制約を広げるものではなく、これらの全経路の非デグレ比較は後続課題に残す。

成功したステージだけを既存の安全退避・DB置換・管理ディレクトリ入替へ渡す。現在状態の完全バックアップと失敗時のDB・画像・設定のロールバックを維持し、成功時にだけ古いセッションを破棄する。既存検証環境を起動する経路は第7版限定のままとし、旧版DBの直置きによる自動更新は許可しない。WebViewの外部通信・ファイル境界・固定17処理の終了保護も維持する。

React設定画面の旧版プレビューと復元確認だけに、移行先第7版・原本非変更・認証引継ぎまたは初期管理者の注意を追加する。C#ホストかつ第1～6版だけに適用し、Tauri版・現行第7版では従来表示を維持する。検証用パッケージは第13段階と対応範囲をmanifest・READMEで一致検査する。試験は独立した合成ルートの実移行SQL由来DBと管理ファイルだけを使い、本番ルートを読まない。

LegacyBackupCheckは716チェックに合格し、全版の全FAQ詳細取得、旧カラム・検索文書・FTS保持、認証・監査補完の境界、長文・深い構造、画像・設定・履歴・採番、拒否前後の現データ・元アーカイブ不変、復元後再起動を確認した。継続保存74件と既存回帰、React 205件、送信JSON境界3件を加えたSecurity 145件、パッケージ22件も合格した。個人用の合成第5・6版バックアップとB1～B4はリポジトリ外で扱い、未実施の実画面受入を自動試験で代替しない。

#### 4.6.10 第14段階：本文の全経路互換と復元確認トークン

第13段階B1～B4は2026-09-05に利用者から全合格を受領した。第14段階は通常の本文検証からC#だけの文字数・ノード数・構造深さ制限を外し、安全な許可構造を維持する。保存本文のJSONコンテナ深さは128までとし、parse済みのJsonElementが渡っても検査する。画像IDの置換・画像を除いたJSON出力などのJsonNode変換でも同じ境界を使い、本文や旧項目を途中で短縮しない。

HostRequestEnvelopeと引数DTOは深さ144、保存本文は128、HostResponseJsonは144とする。16MB要求上限、重複プロパティ・不正Unicode・未知契約拒否、正規送信元照合は維持する。JSON転送のDTO外側も144とし、各本文の128上限と未知項目・画像除外・参照整合・容量を別途検査する。JSON取込は1回読み取った同じバイト列から得た文書をSHA照合・検証・反映に共用し、検証後にファイルを再読込して別内容を適用しない。CSVの本文ハッシュ一致時は元の構造を保持し、不一致の場合だけ従来どおり安全な段落へ置換する。巨大FAQの16MB超要求、Rust側JSONファイル全体128の境界差、Codex専用JSON深さ64を超える本文の修正委譲は本番前の残比較条件とする。

復元プレビューにはC#専用の暗号学的乱数トークンを付与し、認証セッション・正規化した絶対パス・検査したアーカイブ全体SHA-256へ結び付ける。有効期限10分、保管最大16件、起動中メモリだけで扱い、復元試行で消費する。同じ利用者IDでも再ログイン後は別セッションとし、古いトークンを使えない。ホストはトークン必須経路だけを呼び、内部試験用の直接復元経路を画面へ公開しない。

最終復元では書込・削除共有を許可しない同一FileStreamを使い、その実体のSHA-256、ZIP/manifest検証とステージ展開を連続して行う。確認対象の不一致・期限・再使用等は固定`BK-011`で再確認を案内し、安全退避や現DB置換へ進めない。実行中は同じ管理者セッションを保持し、復元途中へ別ログインを割り込ませない。ログイン時もDB認証からセッション代入まで同じセッションロック内で行い、ロック取得順をセッション→DBへ統一する。復元前DBで認証した待機中ログインが復元後に旧セッションを書き戻すことを防ぐ。成功後のセッション破棄、固定17処理の終了保護、ロールバック、元バックアップ非変更を維持する。ReactはC#時だけトークンを渡し、Tauri版へ新しい引数を送らない。

#### 4.6.11 第15段階：Codex専用JSONと履歴の深さ互換

第14段階C1～C4（C2再試験を含む）は2026-09-05に利用者から全合格を受領した。第15段階はCodex専用のシリアライズ・デシリアライズ・重複項目検査・既存履歴の意味比較へ同じ有限境界を適用する。C#だけに残る既定64を使わず、ルート込みのJSONオブジェクト・配列の全体深さ127までとする。現行Rustのserde_json既定予算128では、最初に予算を減算するため全体127が上限となることを実際の提案・委譲DTOでも試験し、C#のMaxDepth128との差を混同しない。

CodexJsonの共通設定を委譲書出し、提案と委譲の読込、事前の重複項目検査、提案履歴の保存・読込、同一受付番号の意味比較で共用する。現在のFAQ本文は通常の保存本文境界128で読み、比較するCodexファイルの全体127と区別する。未知プロパティ拒否、大文字小文字の厳格一致、不正JSON拒否を維持し、JsonElementの受渡しだけで深さ検査を迂回しない。127超の委譲はファイル確定前に拒否し、元FAQや既存委譲を変更しない。

提案の本文は外側のproposal・faqの2コンテナを除く125、委譲本文はdelegation・articles配列・articleの3コンテナを除く124までを生成・受付前に検査する。委譲の深さ超過は既存`CDX-020`で、構造が深すぎることと入れ子を減らす対処を日本語で案内する。保存先の容量・アクセス権エラーと同じ対処文で誤案内しない。

提案1MiB・委譲5MiB、原子的書出し、固定ディレクトリ・通常ファイル検査、許可ノード・属性・HTTP/HTTPSとコピー専用境界は変更しない。修正案の画像属性・位置・順序は委譲元と現在DBの両方に照合し、承認は既存の単一トランザクションと版競合確認を維持する。項目順やUnicodeエスケープ表現だけの違いを改ざん扱いにせず、実際の内容差を同一受付番号で上書きしない。却下・再検討・承認済み重複防止の意味も変更しない。

自動試験は64超の合成本文で修正・統合委譲、提案受付・承認、保存済み履歴、最新版再検討、意味比較、画像不変と競合・容量・深さ拒否を確認する。現行Rustの製品処理は変更せず、試験専用コードで同じコンテナ定義と深いファイルの読込を比較する。通常JSON転送の全体144とRust全体127の差は、このCodex補完の合格範囲へ含めない。

第15段階パッケージは段階番号とCodex深さ127をmanifestとREADMEへ記録し、タイトルを第15段階として表示する。旧段階や深さmetadataの不整合、必須説明の欠落を拒否する。ホスト要求16MiB、本文128、応答144、終了保護17件、復元確認トークン、本番切替保留は維持する。画面や本番プラグインへ試験ファイルを自動投入する機構は追加しない。

#### 4.6.12 第16段階：採用済み最終試験と本番候補

第15段階D1～D4は利用者全合格とし、要件1.15～1.16のF1～F7を採用する。通常利用の非デグレ、既存データ保持、権限・委譲境界、安全拒否と復旧を必須とする。16MiB超要求の成功や通常JSON極深構造のRust逆取込を際限なく拡張することは正式切替条件にしない。既存段階の合格は維持するが、最終候補で未実施の必須項目は未合格のまま追跡する。

`CandidateLaunchPolicy`は引数なしを検証環境、唯一の`--production-candidate`を本番候補とし、それ以外を拒否する。ネイティブ確認は既定「いいえ」。確認後に旧Tauriの固定mutexとhidden windowの両方を使う互換ガードを取得し、取得できなければDBを開かない。C#用の利用者別・セッション間排他は検証版と候補版で分離する。旧Tauriのガードはセッション内であるため、DB接続自体も`locking_mode=EXCLUSIVE`と`BEGIN EXCLUSIVE`を取得し、その接続を終了まで保持する。別セッションの旧版が行う起動前の管理ディレクトリ確認等まで無アクセスになるとは主張しない。プロセスkill、既存ロック解除、任意IPC引数は追加しない。

`ProductionDataRoot`は`LocalApplicationData/jp.local.webknowledgesystem`のみを許可する。既存Tauriの有効な第7版DBはC#マーカーなしでも受け入れるが、空環境以外を再初期化しない。初回だけ固定初期化マーカーを使用する。既存DBは作業コピーで事前検査し、排他取得後も検証する。旧版DBの直置きは拒否し、従来の明示バックアップ復元・ステージ移行を利用する。起動時は固有名の安全バックアップを毎回作り、元DBと管理ファイルを再検証できた場合だけログイン画面へ進む。任意ルートの試験口はinternalかつ厳格なTEMP合成ルートだけに限定する。

`MigrationGateReport.CandidateUseAuthorized`は起動時の明示確認に対応する候補使用の区別であり、`CutoverAllowed`を真にしない。画面・パッケージは正式切替前であることを維持し、通常起動で本番を開かない。共通React/Tiptap/CSS、DB第7版、バックアップ形式第1版、復元後ログアウト、認証・画像・ID・検索の契約は維持する。

第16段階の実行物は`CandidateUseEnabled=false`とし、候補引数を受けても案内後にDB接続前で終了する。C#の異常終了でOS排他が消えた後、未更新の旧Tauriは復元中断記録を見ず起動できるためである。Rustにもデータルートの初期化前・DB接続後に固定の中断記録/初期化中マーカーを調べ、存在・検査失敗時はC#での復旧を案内して止める保護を追加する。記録内容の解析・自動削除・Rustによる巻戻しはしない。このソース修正を既存導入exeへ反映した扱いにはせず、安全な切戻し入口の確認を候補有効化条件へ残す。

起動後のWebView準備失敗は通知後に共通の終了処理へ進み、使用不能な空ウィンドウがDB排他を保持し続けないようにする。残留ブラウザー一時データの終了案内は検証/本番候補を区別し、候補で保存した場合に「本番データへの変更なし」と誤表示しない。

復元はDB置換前に、検証済み安全バックアップの限定ファイル名とSHA-256だけを持つ`restore-pending.json`をflushした作業ファイルから確定する。次回起動は通常処理より先に記録・アーカイブ全体・スキーマ・管理ファイルを検査し、復元前状態へ巻き戻す。DB・画像等の復旧途中停止も再実行可能とし、完了後だけ記録を除去する。記録破損、退避差替え、DB欠落等は`BK-012`で止めて証拠を保持する。同一プロセス内で巻戻しに失敗した場合もDB操作を停止する。本番候補の復旧は最初に既存DBの排他接続を取得し、同じ接続へSQLite Backup APIで戻して、通常使用へ引き継ぐ。自動試験は合成子プロセスの強制停止を使い、実電源断・ディスク故障からの完全回復保証とは区別する。

正規フロントエンドは要求JSONを一度だけ直列化し、そのスナップショットのUTF-8サイズ16MiBと深さ144を送信前に検査する。`SYS-002`／`SYS-003`は待機登録前に返し、編集内容・離脱保護を保持する。送信データの再直列化で値が変わる経路を防ぎ、ホスト側の送信元・深さ・容量・重複項目検査は残す。

候補のFAQ・メール委譲は同じ本番固定ルートへ接続するが、自動送信はしない。メール画面は初期化完了・ログイン後にだけ開く。検証版では依頼文の送信不可案内を維持する。`get-delegation.ps1`と`submit-faq-proposal.ps1`は元JSONを検証後にそのまま出力・確定し、PowerShellのDateTime化による版文字列変更や項目消失を防ぐ。アプリは未知・重複項目を従来どおり拒否する。実コマンドの合成往復と、Codexに登録されたプラグイン・実会話による往復は区別する。

NSISは検証済みペイロードだけを使用する。アプリは導入先にある0バイトの`.knowledgeapp-installation-lock`を読取共有で生存中保持し、導入側は排他的に取得できる場合だけ変更する。ポータブル配置でロックが存在しない場合は従来起動を維持し、異常なファイル・再解析点は拒否する。アンインストールは所有する実行物だけを対象とし、データルート・旧Tauriを変更しない。最終候補のハッシュと試験結果を固定し、対象PC・実データ・実プラグインの不足は自動試験の合格で埋めない。

#### 4.6.13 第17段階：C# 0.5.0正式主系

要件1.17の利用者承認により、今後の主系は.NET 10 / C# / WPF / WebView2とし、画面は既存React / TypeScript / Vite / Tiptap / CSSを継続使用する。本書のTauri/Rust構成は旧版の互換設計として残し、現行のDB・検索・ファイル・バックアップ・パス検証・認証・Codex処理の責務は同じ契約を持つC#層へ読み替える。Rustはバックアップ等の互換回帰に用い、新機能の二重実装を義務としない。

`CandidateLaunchPolicy`は互換ファイル名のみ残し、引数なしを本番、唯一の`--rehearsal`を検証環境とする。`--production-candidate`は拒否する。本番は固定`LocalApplicationData/jp.local.webknowledgesystem.csharp`だけを`KnowledgeDatabase.OpenProduction()`で開く。旧Tauriルートを開かないため、旧exeとの共有mutex取得も不要とし、C#自身の利用者別排他とSQLite排他接続は維持する。初回は空の初期管理者から開始し、利用者が選んだフルバックアップだけを通常の検査・確認トークン・安全退避・復元処理で引き継ぐ。管理IDと内部IDを再採番せず保持する。

`StartupSafetyBackup`は各起動で新規作成し、復元中断記録もC#専用ルートだけへ置く。旧Tauri・検証ルートは自動探索・コピーしない。切替後の正本はC#であり、旧版に戻る場合は切替直前状態となる。以後のC#更新を引き継ぐ必要がある場合は、利用者がC#で最新バックアップを作り旧版の通常復元を明示実行する。自動双方向同期はしない。

`MigrationGateReport`形式2は`ProductionUseAuthorized=true`、`AcceptanceScope=user-approved-small-scale-cutover`、`RecommendedPriority=csharp_primary`をソースで固定する。`CutoverAllowed`は今回の運用承認と全項目受入を区別して扱い、`AllFinalChecksPassed`は未実施項目があればfalseのままとする。DB未起動の検査・配布物では`ProductionDataOpened=false`とし、実際にホストが開いた場合だけtrueにする。画面や外部引数から承認フラグを切り替えるAPIは設けない。

本番画面は試作・切替禁止表示を除き、標題`KnowledgeApp C# 0.5.0`と中立色のC#案内を表示する。検証起動では黄色の検証表示とCodex送信不可案内を残す。FAQのUI/CSSは変更しない。メール委譲、4つのPowerShellコマンド、分類カタログも同一のC#固定ルートを参照し、環境変数の上書きや旧ルートへのフォールバックで接続先を変えない。登録済みプラグインとの実会話確認は合成コマンド試験と区別する。

正式NSISは`LocalApplicationData/Programs/KnowledgeApp-CSharp`、C#専用HKCU登録、スタートメニュー`KnowledgeApp (C#)`を用いる。Version/FileVersionは0.5.0/0.5.0.0。旧Tauriと試作NSISの登録・ファイルは変更しない。所有ファイル検証、UIハッシュ、実行中更新/削除拒否、ランタイム不足案内、利用者データ非削除は維持する。スクリプト内部のTrial名は互換名として残し、公開入口は`New-CSharpReleasePackage.ps1`と`Test-CSharpReleasePackage.ps1`とする。
...
- Web上の外部資料はHTTP/HTTPS参考URLとして表示し、接続先ホスト名と完全なURLを確認してから既定ブラウザーで開く。ブラウザーへ委譲後の内容は利用環境のブラウザー・ネットワーク管理に従う。
...
- FAQ詳細への通常遷移前に、`articleId`と現在表示するタイトルをアプリ内の一時状態へ登録する。同じ`articleId`がある場合は既存タブを選択してタイトルだけを最新化する。
- タブ状態は同一アプリ起動中のReactメモリだけへ保持し、SQLite、設定、履歴、バックアップ、Web Storageへ保存しない。検索条件の復元URLは各詳細画面の遷移状態として従来どおり扱う。
- タブ列はCSS Gridで開いている件数分の`minmax(0, 260px)`列を作り、`justify-content: start`と6px間隔で左から並べる。少数時は列とタブ本体を最大260pxにそろえ、余白をタブ間へ配分しない。1～10件の合計幅が利用可能幅を超える場合は均等に縮小する。横スクロールは設けず、タイトル部分だけを省略表示し、閉じるボタンは各タブ内に残す。完全なタイトルはタブの`title`属性で確認できる。
- FAQ検索・一覧には固定キー`faq-search`、FAQ詳細には`faq-article:{articleId}`を割り当て、現在の`window.scrollY`をReactメモリのMapへ記録する。画面切替前に移動元を保存し、移動先の初期表示時とデータ読込完了後にその画面固有の位置を復元する。
- スクロール位置Mapは同一アプリ起動中だけ保持し、SQLite、Web Storage、設定、検索・閲覧履歴、バックアップ、エクスポートへ保存しない。FAQ検索条件のURLと、検索結果から詳細へ進んで戻る従来の一時復元は維持する。
- タブ数が10件の状態で異なるFAQを開こうとした場合は遷移を中止し、「開いているFAQタブが10件あります。既存のタブを閉じてからもう一度開いてください。」を`role="alert"`で表示する。既存タブを古い順に閉じる処理は行わない。
- 表示中タブを閉じる場合は、閉じる前の配列で右隣、なければ左隣を選択する。最後のタブなら`#/search`へ移動する。非表示中タブを閉じても現在画面を変えない。
- タブを選択したこと自体は閲覧履歴にしない。別のFAQ詳細へ実際に切り替わり、詳細取得が成功した場合だけ既存の閲覧履歴処理を実行する。
...
- ログインは利用者DBの通常Argon2idハッシュだけを照合する。固定緊急認証の定数・分岐を持たない。紛失時のキー照合は通常セッションを生成しない。
...
- `list_codex_proposals`は分類カタログを最新化し、提案箱内の通常ファイルだけを最大1MBまで読む。未知項目を含むJSON、ファイル名と受付番号が異なる提案、任意HTML・危険なリンク、新規・統合時の画像を拒否する。C#移行版では受付時にも委譲番号・種類・元FAQと更新日時を照合し、修正画像のノード属性・位置・順序の維持を検査する。同一受付番号の異内容を含む不正提案は個別に案内し、正常提案の一覧や保存済みの確認待ち履歴を隠さない。
...
- C#移行版はタイトルと概要の前後空白を除いて保存する。既存Rust履歴との再送比較は型付きJSONの意味を比較し、プロパティ順や文字のエスケープが異なるだけでは`CDX-013`にしない。内容が異なる場合は保存済み履歴を保持し、承認は画面に提示した保存済み原文だけから行う。
...
1 FAQは1つの質問・目的に限定する。原則7項目を超える手順、複数目的、複雑な画像・表を要する場合は、FAQ分割またはアプリ外で管理する外部資料への切り分けを利用者へ提案する。簡潔さと正確性・安全性が競合する場合は、正確性・安全性を優先する。
...
- 修正提案の画像ノードは委譲元と同じ属性・位置・順序を維持する。C#移行版はノード属性と本文構造内の位置を検証し、追加・削除・参照変更・代替文変更・移動・並べ替えを`CDX-021`で拒否する。承認時にも現在FAQの画像参照を再検証し、画像本体の入出力を提案反映経路へ追加しない。
...
2. `login`は正規化ログインIDで利用者を取得し、利用停止を拒否してから通常のArgon2idハッシュだけで空欄を含む入力を検証する。成功時はC#メモリに利用者・権限・認証世代・DB実体の世代を保持し、API実行前に失効を検査する。
...
7. 固定緊急ハッシュと追加照合を廃止する。復旧方式は冒頭のC# 0.6.0仕様と「管理者パスワード復旧_実装計画.md」に従い、秘密の平文・復旧限定権限を永続化しない。
...
アプリ外で管理する外部資料本体と外部参考サイトの本文は取り込まず、索引へ含めない。コピー用テキスト枠の参照先文字列と参考URLの表示文字はFAQ回答の一部として通常どおり検索用プレーンテキストへ含める。
...
外部管理のHTML、Excel、PDF、Wordその他の資料本体はKnowledgeAppへ取り込まない。FAQ回答内に次のいずれかで参照先だけを記載する。
...
- 外部資料の配置、ファイル名、アクセス権、更新、安全性、可用性、バックアップは資料管理者側の管理対象とする。
- HTTP/HTTPS参考URLを既定ブラウザーへ委譲した後のJavaScript、外部リソース、認証、ダウンロードは、利用環境のブラウザー・ネットワーク・アクセス制御に従う。
...
| 既定ブラウザーで開く外部資料 | HTTP/HTTPS限定、接続先ホスト名と完全なURLを確認し、委譲後は利用環境のブラウザー・ネットワーク管理に従う。 |
...
| 履歴 | `record_search_log` / `record_article_view` | 利用者操作後の検索履歴・FAQ閲覧履歴を明示記録 |
...
| 専用Windows・対象環境 | 出所を固定した旧版からの更新、合成データ、Excel、外部資料参照、表示倍率、単一起動、バックアップ・復元、アンインストール・再インストール、利用環境の規定 |

2026-09-05の利用者指示により、以後の段階テストはMarkdownを基本とし、Excel形式を必須としない。自動試験、Codexによる実画面操作、利用者確認を別欄に分け、対象、操作・条件、期待結果、判定、根拠、未実施理由または利用者に残る確認を簡潔に記録する。判定は「未実施／合格／不合格／対象外」とし、実画面を操作していない試験を画面合格と記載しない。利用者がExcelを希望する場合や多数の個人記入が必要な場合は、従来のチェックシート形式を継続できる。

認証情報、実FAQ本文、実メール本文、実ファイルパスは記録しない。個人が記入するチェックリストと結果、個人・端末の情報を含む実施記録は形式にかかわらずリポジトリ外へ保存し、Git・配布物・アプリのフルバックアップへ含めない。既存のExcel結果は保存する。ソース管理できるのは、個人情報・実データを含まない試験コード、合成フィクスチャー、設計上の手順と合成試験結果要約だけとする。
...
- 0.4.3を最後にSurface再試験は行わないこと。0.4.4の一人利用試行では対象PCで通常認証、初期管理者緊急認証、既存データ保持、利用者別データルート、ローカルバックアップ・復元を確認し、未署名配布と固定共通認証の正式な運用責任者の判断は複数人・正式配布の再開条件まで延期すること
...
- FAQタブで異なる10件を開き、同一FAQの重複を作らず切り替えられること。11件目では所定の日本語メッセージを表示し、既存タブを自動で閉じないこと
- 10件のFAQタブが横スクロールせず利用可能幅へ均等に縮小され、すべてのタブと閉じるボタンを選択できること
- FAQ検索・一覧と10件のFAQ詳細を異なる縦位置へスクロールし、相互に切り替えても各画面固有の位置へ戻ること。位置がDB、Web Storage、履歴、バックアップへ保存されないこと
- 表示中タブを閉じた場合は隣接タブ、最後のタブならFAQ検索へ移り、タブ開閉だけで閲覧履歴を追加しないこと
- C#同等性レポートは未移植項目が1件でもあれば`cutoverAllowed=false`とし、現行Tauri版を主系として示すこと
- C#認証用合成DBで、初期管理者の空パスワード、NFKCログインID、Argon2id PHC、誤認証、管理者・一般利用者境界、利用停止、自己停止拒否、最後の管理者保護、パスワード再設定、空パスワード方針を自動確認し、固定緊急認証は平文を試験コードへ保存せずWindows手動試験で確認すること
- C#合成DBの第1版から第7版への移行で、既存FAQ、監査担当者、移行前バックアップを保持し、破損DB・未知の将来版・一時フォルダ外のパスを拒否すること
- C#分類・検索用合成DBで、1～5階層と制約、同義語、検索3範囲、NFKC、日本語分解、短語補助、FTS5、重み付け、一致理由、表示対象除外、50件ページング、明示履歴記録を確認すること
- C# FAQ詳細用合成DBで、Tiptap JSON、監査表示、旧検索情報、タグ、関連FAQ、削除・統合表示、異なる10件の読取と、詳細取得成功後だけの閲覧履歴・遷移元検索ログIDを確認すること
- メール走査は明示コマンド時だけ実行され、未選択メール、原本パス、添付、Outlookプロファイルを委譲へ含めないこと
...
4. 配布設定とNSIS出力へDB、FAQ、履歴、添付画像、アプリ外で管理する外部資料、CSV、JSON、バックアップ、Codex提案が含まれないことを自動検査する。
5. インストーラーの版、サイズ、SHA-256、署名状態をリリース試験記録へ残す。
6. 0.3.3～0.4.3のSurface結果は画像不具合の診断・解消証跡として保持し、0.4.3を最後にSurface再試験を終了する。0.4.4の一人利用試行は実FAQへ接続しない検証用Windows利用者から開始し、通常認証、緊急認証、利用者別データルート、ローカルバックアップ・復元を確認してから実FAQへ進む。
7. ソースは`yknk1015/local-knowledge-app`の`main`をPublicで公開し、コミット作成者情報はGitHubの`noreply`へ統一する。複数人利用または正式配布へ移る前に、2026-08-16付の延期事項を再開し、利用環境の規定のコード署名、GitHub公開範囲、配布経路、WebView2・実行制限・ウイルス対策・UNCアクセスを確認する。
...
コード署名は一人利用試行では延期し、取得元コミットまたはインストーラーのSHA-256で出所を固定する。複数人利用または正式配布前に要否を確認し、必要ならリリース工程へ追加する。
...
| C#非デグレ移行 | WPF＋WebView2で現行React/Tiptap UIを再利用し、型付きAPI、SQLite/FTS5、既存データルート、保存形式、業務検証、Repository境界を維持する。全ゲート合格まで現行版を主系とする |

従量課金費用を抑えるため生成AI APIはアプリへ組み込まない。ローカル生成AIモデルも配布サイズ、端末性能、対象PCへの導入・保守負荷を増やすためMVPの対象外とするが、生成AI連携自体を避ける方針ではない。定額利用のCodexによる新規FAQ、既存FAQの修正、選択式統合は専用プラグイン、明示委譲ファイル、確認待ち提案箱で実装する。多数FAQの一括整理はJSONエクスポート・インポート完成後の別機能とする。

C#移行は必須の最終到達点であり、現在は互換ホスト、同等性ゲート、認証・権限、分類・同義語・日本語検索・FTS5、FAQ詳細・最大10タブ、FAQ編集・監査、リッチテキスト・画像添付・外部URL・コピー境界、検索・閲覧履歴、CSV・JSON入出力まで合成DB自動試験と利用者手動試験に合格し、フルバックアップ・検査・復元は合成DB自動試験に合格して利用者手動試験を待つ段階である。標準案は.NET 10 LTS、WPF、WebView2とし、現行React/Tiptap UIを共用する。利用環境が.NET Framework 4.8を指定する場合は同じUI・メッセージ契約で再評価する。ASP.NET案はIIS、共有認証、共有DBまたは複数人同時利用が利用要件になった場合だけ別変更として扱う。UIや機能が制限される移行は本番採用せず、制限解消まで優先度を下げる。
...
| BD-06 | アプリ外で管理する外部資料本体は取り込まず、ローカル・UNCパスはコピー専用、HTTP/HTTPSは確認後の既定ブラウザー表示とする。 | 既存の外部ディレクトリを重複管理せず、HTML以外の業務資料も同じ方式で案内できる。 |
...
| BD-10 | 外部資料本文を取り込まず検索しない。 | アプリ外で管理する資料とKnowledgeAppの責任境界を明確にし、検索索引へ外部内容を混在させない。 |
...
| BD-36 | C#移行はWPF＋WebView2で現行React/Tiptap UIを再利用し、全同等性ゲート合格前は現行Tauri版を主系とする。 | 採用標準言語へ移行しながら、画面・検索・編集・データ保護のデグレードを切替条件で防ぐ。 |
| BD-37 | FAQ詳細タブはReactの起動中メモリだけで最大10件を保持し、横スクロールさせず均等に縮小する。FAQ検索・一覧と各FAQの縦スクロール位置も起動中メモリで画面別に保持し、11件目では既存タブを自動で閉じない。 | すべてのタブを一目で選べる状態と、比較中の各FAQで読んでいた位置を維持しつつ、利用者データや履歴を増やさない。 |
| BD-38 | Outlook履歴は明示操作時のローカル読取、選択・マスク、専用委譲番号の順でCodexへ渡し、原本・添付・未選択メールは渡さない。 | 常時収集と過剰な機密情報送信を防ぎ、既存の確認待ち提案・承認境界を再利用する。 |
| BD-39 | C#認証・権限とDB第1～7版の初期移植は、Rust版と同じSQL・Argon2id・エラー契約を使用し、OS一時フォルダの合成DBだけで自動・手動確認する。 | 本番DBを変更せずに保存形式と権限境界の互換差を検出し、FAQ CRUD・FTS5移植前に基盤を固定する。 |
| BD-40 | C#分類・検索段階は、現行React画面と型付き契約を維持し、分類制約、同義語、FTS5候補抽出、短語補助、最終採点、50件ページング、明示履歴記録を同じSQLite形式で実装する。手動同等性確認までは合成DBだけを使用する。 | 綺麗なUIと検索品質を作り直しで損なわず、候補抽出と業務順位を分離した現行仕様の非デグレを段階判定する。 |
| BD-41 | C# FAQ詳細段階は現行React/Tiptap画面を再利用し、`get_article`と`record_article_view`だけを先に移植する。詳細読取と閲覧記録を分離し、タブ上限判定に失敗したFAQの閲覧履歴を追加しない。画像・表・URL・コピー・添付は別ゲートで判定する。 | 最大10タブの既存UIを維持しながら、表示データと履歴の互換性を小さい読取単位で検証し、未移植操作を合格扱いにしない。 |
| BD-42 | C# FAQ編集段階は現行Reactの4区分と管理一覧を再利用し、基本情報、本文、タグ、状態・表示、管理ID、監査、FTS5を同一トランザクションで更新する。画面から変更しない旧検索情報・関連FAQ・添付行は通常更新で保持し、画像・外部URL起動は別ゲートに残す。 | 綺麗なUIと既存データを損なわずに書込系を小さく移植し、画像など未完了の機能を暗黙に削除・合格扱いしない。 |
| BD-43 | C#リッチテキスト・添付段階は、画像を合成データルートの一時領域から保存成功後だけ確定し、DB失敗時は新規ファイルを戻す。添付付き複製は新UUID・別ファイルとし、本文には管理参照だけを保存する。画像専用仮想ホスト、HTTP/HTTPS再検証、プレーンテキスト書込専用クリップボードを型付き境界として公開する。 | 現行Tiptap UIと画像・表・URL・コピー操作を維持しながら、絶対パス保存、任意ファイル公開、外部URL自動実行、画像共有による連鎖削除を防ぐ。 |
| BD-44 | C#フルバックアップ段階は現行React画面とformat version 1を共用し、管理者だけにWPF専用ダイアログと作成・検査・復元の固定契約を公開する。DBスナップショットと管理ファイルをmanifestのサイズ・SHA-256・版で検証し、復元前安全バックアップ、DB・管理フォルダの巻戻し、成功後ログアウトを必須とする。 | 画面と既存バックアップ形式を維持しながら、任意ファイルAPI、破損復元、部分確定、認証セッションの持越しを防ぐ。 |
...
| FR-MAIL Outlook履歴 | 2.5、3.2、4.6、13、18 |
...
- `ローカルFAQデータ・Git除外詳細設計書.md` v0.82
- [文書整理ガイド](文書整理ガイド.md)
- [今後の課題](今後の課題.md)
...
- [.NETのリリースとサポート](https://learn.microsoft.com/ja-jp/dotnet/core/releases-and-support)
- [WindowsアプリでのWebView2](https://learn.microsoft.com/ja-jp/windows/apps/develop/ui/controls/webview2)
...
- [NuGet：MsgReader](https://www.nuget.org/packages/msgreader)
- [NuGet：OpenMcdf](https://www.nuget.org/packages/OpenMcdf)
- [Agent-friendly CLI](https://learn.chatgpt.com/use-cases/agent-friendly-clis)
