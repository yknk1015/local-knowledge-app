...
14. FAQマスコットは同梱静的素材と共通App Shell内の独立部品で構成し、主要操作を妨げない小さい常駐表示、ランダム動作、キーボード操作、動きを減らすOS設定への対応を両立する。
...
- 白を基調に緑・青を切り替えられる全画面カラーテーマ、共通メニュー・分類絞り込み・検索結果の3領域化、横幅に応じた2列／1列FAQカード
- FAQ編集画面の3区分ナビゲーション、入力状況表示、項目別エラー、公開状態カード、用途別回答ツールバー、常時確認できる保存バー
...
- 金縁の片眼鏡と紺色の角帽を持つちびフクロウの透明ドット絵スプライトシート、画面右下の常駐表示、クリック時のランダム動作、キーボード・動きを減らす設定への対応
...
| モジュール | 主な責務 |
|---|---|
| App Shell | 共通ヘッダー、左ナビゲーション、画面遷移、カラーテーマとタイトル分類表示設定の読込・共有、FAQマスコットの常駐表示 |
| Search | 分類選択、検索条件、検索結果、ページング、トップ分類名を付けるタイトル表示 |
| Article View | FAQ詳細、関連FAQ、手順書、参考URL、コピー用テキスト枠 |
| Article Editor | 入力、リッチテキスト、画像、表、コピー用テキスト枠、保存前検証 |
| Category Manager | 追加、名称変更、移動、並べ替え、削除 |
| Synonym Manager | 同義語グループの管理 |
| History | 検索履歴、0件検索、閲覧履歴 |
| Backup & Settings | カラーテーマとタイトル分類表示の選択、フルバックアップ、復元、JSON入出力、リンク検査 |
...
│ FAQの管理    │ すべて       │ FAQカード  FAQカード                 │
│ 分類の管理   │ ├ PC関連     │ FAQカード  FAQカード                 │
│ 設定・情報   │ └ プログラム │                                      │
└──────────────┴──────────────┴──────────────────────────────────────┘
```
...
- 全画面は白い操作面を基調とし、設定で選択した緑または青を主要操作、選択、フォーカスの強調色に用いる。十分な横幅ではFAQカードを2列、狭い場合は1列にする。
...
共通App Shellの最後に`FaqMascot`を1つ配置し、画面右下へ固定表示する。表示寸法は96×104ピクセルを基準とし、操作用ボタン領域を含めても108×116ピクセルに収める。ダイアログより低い重なり順とし、主要操作を隠しにくい余白へ置く。

静的素材は`src/assets/mascot/faq-owl-spritesheet.webp`へ置く。アトラスは8列×9行、1セル192×208ピクセル、全体1536×1872ピクセルとし、未使用セルを完全透過にする。画面では50%へ整数縮小し、`image-rendering: pixelated`でドット輪郭を維持する。

| 行 | 状態 | 用途 |
|---:|---|---|
| 0 | `idle` | 静かな待機、瞬き、小さい上下動 |
...
- 通常は`idle`をループする。クリックまたはボタンのEnter・Space実行時は`waving`、`jumping`、`waiting`、`running`、`review`から、直前と異なる1つを等確率で選び、1周期後に`idle`へ戻す。
...
- `prefers-reduced-motion: reduce`の場合は`idle`の連続コマ送りを停止し、クリック時も選択状態を600ミリ秒だけ表示して待機へ戻す。
...
初回のメイン画面UI改善では、共通メニュー、分類絞り込み、検索・結果を独立した領域として配置する。FAQカードは分類、状態、新着・更新、タイトル、概要、重要度、更新日の順で表示し、カード全体から詳細へ移動できる。分類の選択件数は直接所属件数を補助表示し、検索結果件数は選択分類の配下を含む実際の結果件数を表示する。タイトル分類表示がONの場合は、一覧取得済みの分類IDと親IDをたどってトップ分類名を求め、タイトルを`【トップ分類名】タイトル本体`として描画する。保存済みタイトルと検索索引は変更しない。
...
10. 横幅が十分な場合は結果を2列カードで表示し、カード幅が狭くなる画面では1列へ切り替える。
11. 実行済みの検索語、選択分類、下書き・廃止表示はURLの検索パラメーターへ保持する。FAQカード選択時は検索画面のURLとスクロール位置を遷移情報として保持し、SCR-02の「一覧へ戻る」で同じ条件を再検索した後、元の表示位置を復元する。
12. 検索条件欄に「条件を初期化」を常設する。実行時は検索語を空、分類を仮想ルート「すべて」、下書き・廃止表示をオフへ一括変更して初期一覧を再取得し、既に初期状態の場合は無効表示する。
...
検索URLは画面遷移中の表示状態を再構成するために用い、FAQ本文や検索履歴の正本にはしない。スクロール位置はWebViewの`sessionStorage`へ一時保存し、復元後またはアプリ終了時に破棄する。SQLite、バックアップ、エクスポート、ソース管理対象のファイルへは保存しない。
...
- 初回実装で入力可能な「基本情報」「回答」「公開・表示設定」を1画面内の独立したカードとして上から順に配置する。未実装の検索情報・関連情報は、入力機能の実装時に同じ区分へ追加する。
- 画面上部に3区分のナビゲーションを置き、各区分へスクロール移動できるようにする。必須情報や回答の入力状況を番号またはチェックで表示するが、区分を順番に完了しないと次へ進めないウィザードにはしない。
- 基本情報では、タイトルと概要の文字数、分類・重要度の説明、公開時必須となる項目を入力欄の近くに表示する。
...
- `get_settings`で`app_settings`から起動時に読み込み、`save_settings`で保存する。保存に失敗した場合は直前のテーマへ戻し、`SET`エラーを表示する。
...
分類、状態、非表示、更新日時、論理削除状態に索引を設ける。DBスキーマ第2版で表示フラグ3列、第3版で分類説明と`codex_proposal_receipts`、第4版で`codex_proposal_history`を追加する。第1～3版DBおよび旧バックアップは、起動・復元時に未適用マイグレーションを順番に実行して第4版へ移行する。既存FAQは変更せず、既存分類の説明は空文字とする。コピー用テキスト枠ではDB列を追加せず、保存または更新したFAQの`body_format_version`を2とする。第1版本文は第2版でもそのまま読取可能なため、既存FAQの一括更新やDBマイグレーションは行わない。
...
表示設定はキー`appearance`、値`{"colorTheme":"green","showTopCategoryInTitle":true}`として保存する。カラーテーマは`green`または`blue`、タイトル分類表示は真偽値とする。未保存時は`green`かつタイトル分類表示ONを既定値とし、旧JSONに`showTopCategoryInTitle`がない場合もONとして読み込む。未知の項目や値はRustで受け付けない。既存テーブルを使用するためDBマイグレーションは追加しない。`app_settings`はSQLiteスナップショットに含まれるため、フルバックアップと復元の対象になる。
...
6. 前回の画面サイズ、分類ツリー幅、既定バックアップ先、カラーテーマ、タイトル分類表示設定を復元する。未保存時はグリーンかつタイトル分類表示ONを使用する。
...
| React部品 | 検索条件、エディター、確認ダイアログ、Codex提案・分類選択、カラーテーマ選択・即時反映、タイトル分類表示のON/OFF、FAQマスコットのランダム動作・待機復帰・キーボード操作 |
| Rust単体 | 正規化、採点、パス検証、バックアップ名、Codex提案形式検証 |
| DB統合 | 階層移動、5階層制限、CRUD、索引、履歴、Codex重複取込防止、カラーテーマ・タイトル分類表示の既定値と保存 |
...
- 1280×720でメイン画面の共通メニュー、分類絞り込み、検索欄、新規FAQ操作、2列FAQカードが判別でき、フォーカス表示と文字によって選択状態を確認できること
- FAQ編集画面で3区分の移動、公開状態と表示フラグの区別、項目別エラー、回答ツールバー、追従保存バーをマウスとキーボードの両方で操作できること
...
- 旧`appearance`設定JSONにタイトル分類表示項目がなくてもONとして読み込み、選択値を再起動・フルバックアップ・復元後も保持すること
...
- FAQマスコットが全画面共通の右下へ表示され、クリックごとに直前と異なる5種類の候補から動作を選び、所定の1周期後に待機へ戻ること
- FAQマスコットをEnter・Spaceで実行でき、`prefers-reduced-motion: reduce`では待機ループと連続コマ送りを抑制すること
- 配布物に`faq-owl-spritesheet.webp`が含まれ、利用者FAQの添付画像、DB、バックアップ、エクスポートが混入しないこと
...
1. 自動テストと型検査を実行する。
2. UTF-8・LF検査を実行する。
3. 禁止ファイル・機密情報の混入検査を行う。
4. Windows 11 64ビットでリリースビルドを作成する。
5. NSISインストーラーを作成する。
6. 新規インストール、上書き更新、アンインストールを確認する。
7. バックアップ・復元を検証用データで確認する。
8. 変更内容と既知の問題を添えてGitHub等へリリースする。
...
| BD-21 | メイン画面は白を基調に、共通メニュー・分類絞り込み・検索結果を分離し、横幅に応じてFAQカードを2列または1列で表示する。 | 検索範囲と主要操作を見つけやすくし、業務アプリとしての一覧性と読みやすさを両立する。 |
| BD-22 | FAQ編集画面は入力を隠すウィザードにせず、3区分を1画面で確認できる構成とし、入力状況ナビゲーションと追従保存バーを併用する。 | 回答を確認しながら基本情報や公開方法を調整でき、長い画面でも現在の入力状況と保存操作を見失わないようにする。 |
...
| BD-28 | FAQマスコットは8列×9行の透明WebPアトラスを同梱し、共通App Shellの右下で待機・クリック時ランダム動作を行う。 | 小さい常駐表示でもキャラクターの一貫性を保ち、画面ごとの重複実装や利用者データとの混在を避ける。 |
...
- `AGENTS.md`
- `FAQシステム要件定義書.md` v1.8
- `ローカルFAQデータ・Git除外詳細設計書.md` v0.19
