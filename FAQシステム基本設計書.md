...
|---|---|
| 文書名 | FAQシステム基本設計書 |
| 版 | 1.73（C#正式版確定・切替完了記録） |
| 作成日 | 2026-08-08 |
| 上位文書 | `FAQシステム要件定義書.md` v1.52 |
| 対象リリース | MVP（初回リリース） |
...
旧Tauri/Rustの構成説明と段階ごとの切替保留は履歴として残し、現行構成は4.6.13を優先する。旧ソースを今後リポジトリ外へ保管する場合も、C#が参照する移行SQL、Rust相互比較試験、NSIS準備等の依存を先に分離し、必要な互換回帰を失わないこと。本変更ではその分離を実装せず、アプリ・DB・配布物は変更しない。
