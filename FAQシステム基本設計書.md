...
- FAQ編集画面の3区分ナビゲーション、入力状況表示、基本情報の入力単位カード、項目別エラー、公開状態カード、用途別回答ツールバー、常時確認できる保存バー
...
分類の並べ替え、複数値項目、関連FAQ、同義語、日本語検索、履歴画面、JSON入出力は後続の開発単位で実装する。外部資料参照はコピー用テキスト枠とHTTP/HTTPS参考URLで実装済みとし、専用HTML手順書管理は2026-08-16の方針変更により対象外とする。CSV入出力、ローカル認証、利用者管理、FAQ作成者・更新者は2026-08-16の開発単位で実装する。本節は進捗を示すものであり、後続機能の要件を変更するものではない。
...
| Article View | FAQ詳細、関連FAQ、外部資料の参照先、参考URL、コピー用テキスト枠 |
...
| Backup & Settings | カラーテーマ、タイトル分類表示、マスコット表示の選択、フルバックアップ、復元、将来のJSON入出力 |
...
| 関連情報 | 関連FAQ。外部資料参照は回答内のコピー用テキストまたは参考URLとして扱う。 |
...
| 操作 | 新規、編集、複製、CSVエクスポート、CSVインポート、公開、廃止、統合解除、削除、復元 |
...
- パスワード再設定でも同じ方針を適用し、OFF時の空欄は画面とRust側の両方で拒否する。設定切替だけでは既存利用者のハッシュを変更しない。
...
2. `login`は正規化ログインIDで有効利用者を取得し、Argon2で空欄を含む入力パスワードを検証する。成功時だけ`AppState.session`へID・表示名・権限を保持し、最終ログイン日時を更新する。
...
| 入出力 | `export_json` / `import_json` | 将来のJSON移行 |
...
| ファイル統合 | CSV BOM・CRLF・固定拡張子・SHA-256差替え検知・取込前バックアップ、画像、バックアップ、復元、分類カタログ、提案箱 |
...
- DB第1～5版から第6版への起動時移行、初期管理者・既存FAQ監査者補完、旧バックアップ復元後の移行
...
- FAQ編集画面で3区分の移動、基本情報の入力単位カードと必須・任意・公開時必須の文字表示、公開状態と表示フラグの区別、項目別エラー、回答ツールバー、追従保存バーをマウスとキーボードの両方で操作できること
...
```text
MAJOR.MINOR.PATCH
例：0.3.3（CSV用FAQ・分類管理IDを含むプロトタイプ）
```
...
- `ローカルFAQデータ・Git除外詳細設計書.md` v0.39
