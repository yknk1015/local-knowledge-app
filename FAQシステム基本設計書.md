...
|---|---|
| 文書名 | FAQシステム基本設計書 |
| 版 | 1.74（C#移行SQL所有・旧版互換試験とCIの分離） |
| 作成日 | 2026-08-08 |
| 上位文書 | `FAQシステム要件定義書.md` v1.53 |
| 対象リリース | MVP（初回リリース） |
| 仮称 | KnowledgeApp（正式名称決定後に置換する） |

### 現在の正式構成（2026-09-06確定）

正式版はC# 0.5.0（.NET 10 / WPF / WebView2 / React / Tiptap / SQLite FTS5 / 利用者単位NSIS）である。利用者による今回のPCの復元完了報告を受け、切替完了を記録する。以後のバックエンド改修はC#へ実装し、UIを維持する。会社環境等の未実施確認とは区別する。
...
| 設計・試験項目 | 再開条件 |
|---|---|
| コード署名・正式配布・公開範囲 | Publicリポジトリは利用者データと会社資料を除外して初回公開する。利用者追加、正式社内配布、会社情報を含む運用、または会社規定による要求時は公開範囲と配布条件を再確認する |
...
メール履歴は会社情報を含み得るため、検索対象や常時収集元へ追加しない。利用者がその都度「メール履歴を確認する」を実行した場合だけ、C#のローカルメール取込サービスを起動する。初期版は`.msg`を対象にし、`.pst`はランダムアクセス、暗号化、破損、文字コード、巨大ファイル、Outlook版差および会社規定を別ゲートで確認してから追加する。
...
検証用パッケージは新しい出力先へ必要な実行物・依存物・UIだけを組み立て、最終ツリーの許可リストとハッシュで検査する。既存出力を再利用・上書きせず、DB、WAL/SHM、委譲・提案、エクスポート、添付、ログ、ブラウザープロファイルを含めない。実行環境の前提と依存物の情報を添付し、利用者による配置変更後の起動を確認する。フォルダーパッケージの合格だけではNSIS・会社配布・更新・再導入の合格にしない。`single_instance_distribution`ゲート全体は後続確認まで未合格を維持する。
...
既存のフルバックアップと明示復元は同じ管理データ・形式・安全退避・ログアウトを維持する。検証環境の初期化マーカーとブラウザープロファイルはバックアップ対象へ加えず、復元によってマーカーを消去・再生成しない。パッケージのモードと説明を継続保存の検証用へ更新し、FAQ・DB・画像・プロファイル・マーカーを同梱しない。実プラグインの接続先切替、NSIS、会社配布、本番コピー比較は後続ゲートへ残す。
...
移行後に現行版・必須テーブル・外部キー・有効管理者、分類5階層・循環、許可リッチテキストとFAQごとの添付参照、添付情報とmanifest、既知の表示設定を検査する。未知の旧設定と互換ファイルは不透明データとして保持し、参照先の会社資料へアクセスしない。本文JSON・旧検索情報・検索文書・FTSを再生成せず、第7版の管理ID採番は元SQLに委ねる。移行前後の検査失敗は`BK-006`で拒否する。
...
NSISは検証済みペイロードだけを使用する。アプリは導入先にある0バイトの`.knowledgeapp-installation-lock`を読取共有で生存中保持し、導入側は排他的に取得できる場合だけ変更する。ポータブル配置でロックが存在しない場合は従来起動を維持し、異常なファイル・再解析点は拒否する。アンインストールは所有する実行物だけを対象とし、データルート・旧Tauriを変更しない。最終候補のハッシュと試験結果を固定し、会社PC・実データ・実プラグインの不足は自動試験の合格で埋めない。
...
- タブ列はCSS Gridで開いている件数分の`minmax(0, 1fr)`列を作り、1～10件を利用可能幅へ均等配置する。横スクロールは設けず、タイトル部分だけを省略表示し、閉じるボタンは各タブ内に残す。完全なタイトルはタブの`title`属性で確認できる。
...
認証情報、会社FAQ本文、実メール本文、実ファイルパスは記録しない。個人が記入するチェックリストと結果、個人・端末の情報を含む実施記録は形式にかかわらずリポジトリ外へ保存し、Git・配布物・アプリのフルバックアップへ含めない。既存のExcel結果は保存する。ソース管理できるのは、個人情報・実データを含まない試験コード、合成フィクスチャー、設計上の手順と合成試験結果要約だけとする。
...
7. ソースは`yknk1015/local-knowledge-app`の`main`をPublicで公開し、コミット作成者情報はGitHubの`noreply`へ統一する。複数人利用または正式配布へ移る前に、2026-08-16付の延期事項を再開し、会社規定のコード署名、GitHub公開範囲、配布経路、WebView2・実行制限・ウイルス対策・UNCアクセスを確認する。
...
| BD-36 | C#移行はWPF＋WebView2で現行React/Tiptap UIを再利用し、全同等性ゲート合格前は現行Tauri版を主系とする。 | 会社標準言語へ移行しながら、画面・検索・編集・データ保護のデグレードを切替条件で防ぐ。 |
...
| BD-38 | Outlook履歴は明示操作時のローカル読取、選択・マスク、専用委譲番号の順でCodexへ渡し、原本・添付・未選択メールは渡さない。 | 常時収集と過剰な会社情報送信を防ぎ、既存の確認待ち提案・承認境界を再利用する。 |
...
11. 会社向け正式配布時にPublicリポジトリを継続できるかとコード署名要否
...
- `AGENTS.md`
- `FAQシステム要件定義書.md` v1.44
- `ローカルFAQデータ・Git除外詳細設計書.md` v0.67
