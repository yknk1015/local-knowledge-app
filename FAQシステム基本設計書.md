...
分類の名称変更・移動・削除、FAQの複製・論理削除・復元、画像、複数値項目、関連FAQ、同義語、日本語検索、参考URLの確認付き外部オープン、履歴画面、HTML手順書、バックアップは後続の開発単位で実装する。参考URLは確認付き外部オープンが完成するまでクリック遷移を停止する。本節は進捗を示すものであり、後続機能の要件を変更するものではない。
...
バックアップ名の初期値は`KnowledgeApp_yyyyMMdd_HHmm`とし、利用者が変更できる。拡張子は`.faqbackup`とする。

#### 復元

- バックアップファイル選択
- 作成日時、アプリ版、DB版、FAQ数、手順書数のプレビュー
- 現在状態の事前退避有無
- 「復元」ボタン
...
前回成功した保存先を端末内設定へ保持する。接続できない場合は保存先を選び直せるようにし、FAQの通常利用は継続できる。
