export function legacyBackupNotice(schemaVersion: number, csharpHost: boolean): string | null {
  if (!csharpHost || !Number.isInteger(schemaVersion) || schemaVersion < 1 || schemaVersion > 7) return null;
  const migration = `DB第${schemaVersion}版を第8版へ移行して復元します。元のバックアップは変更しません。`;
